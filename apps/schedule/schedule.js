...
(function () {
    ...
    function isSchedulePending(item) {
        return !!(item && (item.schedulePending === true || item.version === '???' || !item.date));
    }
    ...
    function compareScheduleItems(a, b) {
        const aPending = isSchedulePending(a);
        const bPending = isSchedulePending(b);
    ...
    function getScheduleYearLabel(item) {
        if (isSchedulePending(item)) return t('schedulePendingGroup', '예정');
    ...
    function generateFullSchedule() {
        ...
        if (window.ReleaseScheduleData.autoGenerateCharacters && window.ReleaseScheduleData.autoGenerateCharacters.length > 0) {
            window.ReleaseScheduleData.autoGenerateCharacters.forEach((item) => {
                ...
                const pendingDate = item.schedulePending === true || item.version === '???';
                const releaseDate = pendingDate ? null : (explicitDate ? parseDate(explicitDate) : lastDate);
                releases.push({
                ...
                // Calculate next date after pushing. Undated placeholder entries should
                // not consume the release cadence.
                if (releaseDate) {
    ...
    function getLimitIconPath(isLimit) {
        return `${BASE_URL}/assets/img/character-detail/${isLimit ? 'limit.png' : 'limit_non.png'}`;
    }

    function renderLimitTransitionBadge(transitions) {
        if (!Array.isArray(transitions) || transitions.length === 0) {
            return '';
        }

        return transitions.map((transition) => {
            if (!transition || !Array.isArray(transition.characters) || transition.characters.length === 0) {
                return '';
            }

            const fromLabel = transition.from ? t('limitStateLimited') : t('limitStateStandard');
            const toLabel = transition.to ? t('limitStateLimited') : t('limitStateStandard');
            const labelText = transition.from === true && transition.to === false
                ? t('limitTransitionLimitedToStandard')
                : `${fromLabel} → ${toLabel}`;

            const characterIconsHtml = transition.characters.map((charName) => {
                const displayName = getCharacterName(charName);
                const charUrl = buildCharacterDetailUrl(charName);
                return `
                    <a href="${charUrl}" class="limit-transition-character-link" onclick="event.stopPropagation();" title="${displayName}">
                        <img
                            class="limit-transition-character-icon"
            ...
            return `
                <div class="content-badge limit-transition-badge" title="${t('limitTransitionTitle')}: ${labelText}">
                    <img class="limit-transition-state-icon" src="${getLimitIconPath(transition.from)}" alt="${fromLabel}" title="${fromLabel}">
                    <span class="limit-transition-arrow" aria-hidden="true">
                        <svg viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M4 10H15" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
                            <path d="M10.5 5L16 10L10.5 15" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                    </span>
                    <img class="limit-transition-state-icon" src="${getLimitIconPath(transition.to)}" alt="${toLabel}" title="${toLabel}">
                    <div class="limit-transition-characters">
                        ${characterIconsHtml}
                    </div>
                </div>
            `;
    ...
    function renderCharacter(charName, clickDisabled = false) {
        const charData = window.characterData?.[charName];
        if (!charData) {
        ...
        const codenameHtml = codename
            ...
            : '';
        const disabledClass = clickDisabled ? ' click-disabled' : '';
        const clickAttr = clickDisabled ? '' : ` onclick="window.location.href='${charUrl}'"`;

        return `
            <div class="character-item${disabledClass}"${clickAttr} title="${displayName}">
                <img 
    ...
    function renderReleaseCard(release) {
        ...
        const pendingDate = isSchedulePending(release);
        const dateHtml = pendingDate
            ? `<span class="release-date">${t('schedulePendingDate', '[예정]')}</span>`
            : `<span class="release-date">${formatDate(release.date)} · ${daysText}</span>`;

        const charactersHtml = release.characters
            .map(char => renderCharacter(char, release.click_disable === true))
            .join('');
        ...
        // 콜라보 이벤트 아이콘 (v0.0 제외)
        let collaboIconHtml = '';
        if (release.version !== '0.0' && release.schedulePending !== true) {
            const charData = window.characterData || {};
            let hasP5 = false;
            let hasP5R = false;
            let hasP3 = false;
        ...
        const limitTransitionHtml = renderLimitTransitionBadge(release.limitTransitions);
    ...
    function renderCurrentBanner(releases) {
        ...
        const currentRelease = releases.find(r => r.date && getStatus(r.date) === 'current');
    ...
    function renderTimeline(releases, filter = 'all') {
        ...
        const releasedReleases = releases.filter(r => r.date && getStatus(r.date) === 'released');
        const nonReleasedReleases = releases.filter(r => !r.date || getStatus(r.date) !== 'released');
