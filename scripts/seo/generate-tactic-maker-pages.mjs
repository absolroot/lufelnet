...
/**
 * generate-tactic-maker-pages.mjs
 *
 * Generates static SEO pages for the tactic-maker app in each supported language.
 *
 * Usage:
 *   node scripts/seo/generate-tactic-maker-pages.mjs
 *   node scripts/seo/generate-tactic-maker-pages.mjs --check
 */
...
function ensureSeoMetaShape(meta) {
  for (const lang of LANGS) {
    ...
    if (typeof langMeta.title !== 'string' || !langMeta.title) {
      throw new Error(`i18n/pages/tactic-maker/seo-meta.json missing valid title for ${lang}.`);
    }
    if (typeof langMeta.description !== 'string' || !langMeta.description) {
      throw new Error(`i18n/pages/tactic-maker/seo-meta.json missing valid description for ${lang}.`);
    }
  }
}

function renderTacticMakerPage({ lang, title, description }) {
  const permalink = `/${lang}/tactic-maker/`;
  ...
  return [
    ...
    'layout: default',
    'custom_css: []',
    'custom_js: [tooltip, character/spoiler-state, character/character-list]',
    'custom_data: [/data/character_info.js, revelations/revelations.js, wonder/skills.js, wonder/weapons.js]',
...
function buildExpectedFiles(seoMeta) {
  ...
  for (const lang of LANGS) {
    ...
    const content = normalizeNewline(
      renderTacticMakerPage({ lang, title, description })
    );
...
function runCheck(expectedFiles) {
  ...
  if (missing.length || changed.length || extra.length) {
    console.error('Tactic Maker SEO pages are out of date.');
    if (missing.length) {
  ...
  console.log(`Tactic Maker SEO pages are up to date (${expectedFiles.size} files).`);
}
...
function runGenerate(expectedFiles) {
  ...
  console.log(`Generated ${expectedFiles.size} tactic-maker SEO files in ${toPosix(path.relative(ROOT, OUTPUT_DIR))}`);
}
