...
/**
 * generate-inflation-pages.mjs
 *
 * Generates static SEO pages for the inflation page in each supported language.
 *
 * Usage:
 *   node scripts/seo/generate-inflation-pages.mjs
 *   node scripts/seo/generate-inflation-pages.mjs --check
 */
...
const OUTPUT_DIR = path.join(ROOT, 'pages', 'inflation');
const LANGS = ['kr', 'en', 'jp', 'cn'];
const IMAGE_PATH = '/assets/img/home/SEO.png';

const seoMetaPath = path.join(ROOT, 'i18n', 'pages', 'inflation', 'seo-meta.json');
...
function ensureSeoMetaShape(meta) {
  for (const lang of LANGS) {
    ...
    if (typeof langMeta.title !== 'string' || !langMeta.title) {
      throw new Error(`i18n/pages/inflation/seo-meta.json missing valid title for ${lang}.`);
    }
    if (typeof langMeta.description !== 'string' || !langMeta.description) {
      throw new Error(`i18n/pages/inflation/seo-meta.json missing valid description for ${lang}.`);
    }
  }
}

function renderInflationPage({ lang, title, description }) {
  const permalink = `/${lang}/inflation/`;
...
function buildExpectedFiles(seoMeta) {
  ...
  for (const lang of LANGS) {
    ...
    const content = normalizeNewline(renderInflationPage({ lang, title, description }));
...
function runCheck(expectedFiles) {
  ...
  if (missing.length || changed.length || extra.length) {
    console.error('Inflation SEO pages are out of date.');
    if (missing.length) {
  ...
  console.log(`Inflation SEO pages are up to date (${expectedFiles.size} files).`);
}
...
function runGenerate(expectedFiles) {
  ...
  console.log(`Generated ${expectedFiles.size} inflation SEO files in ${toPosix(path.relative(ROOT, OUTPUT_DIR))}`);
}
