...
ritualDetails.en[0] = "After obtaining Kotone Shiomi, during battle, for every Strategist present (including Wonder's currently equipped Persona), increase party's final damage amplification by 1% (takes effect even when Kotone Shiomi is not in battle). At the start of battle, grant the Arcana Link state to the Sweeper or Assassin ally with the highest Attack. At the start of Kotone Shiomi's turn, she can reselect the target of Arcana Link. Multiple allies cannot be in the Arcana Link state at the same time.\nWhen the ally in the Arcana Link state uses a skill, or when Kotone Shiomi uses a skill on the ally in the Arcana Link state, increase the target's Lunar Bond (max 10). Also, gain the following effects based on current Lunar Bond.\n1: Increase Attack by 50%.\n5: Increase pierce rate by 15%.\n10: Increase critical damage by 50%.\nWhen reselecting the target of Arcana Link, Lunar Bond is reset.\nAlso, Kotone Shiomi can activate Go for Broke once per battle. When activated, she enters the Fortune state.";
ritualDetails.en[1] = "When entering the Fortune state, immediately activate Highlight once without spending the Highlight gauge.\nWhen granting an ally the Arcana Link state, grant them 1 permanent Powerful Bond stack while they are in the Arcana Link state, and set Lunar Bond to 5.\nIf an ally has 3 Powerful Bond stacks after receiving Lyre's Melody, increase the target's critical damage by 30% more for 5 turns.";
ritualDetails.en[2] = "When in the Fortune state, extend the duration of all effects granted by Kotone Shiomi's skills and Highlight by 1 more turn. Also, the copy multiplier when using Lunar Phaseshift is multiplied by 1.25.";
ritualDetails.en[6] = "Go for Broke can be used 1 more time.\nWhen using Lunar Phaseshift on allies other than Wonder, if Wonder is not in the Arcana Link state, also copy certain buffs' effects granted by Wonder to the Arcana Link ally with a copy multiplier of their original effectiveness. These effects last for 1 turn, and certain effects cannot be copied.";
ritualDetails.jp[0] = "汐見琴音を獲得すると、戦闘中、場にいる優越ロールの味方１体ごと（主人公のメインペルソナも対象）に、味方全体の最終ダメージ増幅が１%上昇する（自身が戦闘に参加していない場合でも有効）。戦闘開始時、攻撃力が最も高い支配／反抗ロールの味方を『月下の仲間』状態にする。自身のターン開始時、『月下の仲間』の対象を再選択できる。複数の味方を同時に『月下の仲間』状態に選択することはできない。\n『月下の仲間』状態の味方がスキルを使用、または自身が『月下の仲間』状態の味方にスキルを使用した時、対象の『ルネーション』が増加する（最大１０まで）。また、『ルネーション』に応じて以下の効果を獲得する。\n「１：攻撃力が５０%上昇」\n「５：貫通が１５%上昇」\n「１０：クリティカルダメージが５０%上昇」\n『ルネーション』は『月下の仲間』を再選択するとリセットされる。\nさらに汐見琴音は戦闘中に一度だけ、『全力全開』を発動できる。発動すると『運命の輪』状態になる。";
ritualDetails.jp[1] = "『運命の輪』状態になった時、即座にハイライトゲージを消費しないハイライトを１回発動する。\n味方を『月下の仲間』状態にした時、対象が『月下の仲間』状態中に永続する『絆の力』を１つ付与し、『ルネーション』が５になる。\n味方に『琴奏の調べ』を使用後に対象の『絆の力』が３つの場合、５ターンの間、追加で対象のクリティカルダメージが３０%上昇する。";
ritualDetails.jp[2] = "『運命の輪』状態の時、自身がスキル／ハイライトで付与する全ての効果の持続ターンが追加で１ターン延長される。また『焔華・緋月昂揚』を使用する時の複製倍率が１.２５倍になる。";
...
ritualDetails.jp[6] = "『全力全開』の使用回数が１回追加される。\n主人公以外の味方に『焔華・緋月昂揚』を使用した時、主人公が『月下の仲間』状態でなければ、追加で主人公が『月下の仲間』状態の味方に付与した特定の強化効果を元の効果量の複製倍率で複製する。この効果は１ターン持続し、一部の効果は複製されない。";
