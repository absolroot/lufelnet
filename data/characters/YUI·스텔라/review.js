...
설명은 상당히 복잡하지만 **실제 운영 방식은 단순한 편**으로 설명을 신경쓰지 않고 사이클 순서대로 진행하면 무리없게 구성된다.
...
`,
review_cn: `
**一名把总攻击数值当作主要输出系数来使用的核热单体输出**。

虽然文本说明看起来很复杂，但**实际操作反而相当直观**，基本只要按循环顺序行动就能顺利打完。

- 每回合开始时会自动召唤1名骑士，使用技能2（冷却1回合）时也会再召唤1名骑士。
  - 茄子 → 土豆 → 蘑菇 → 芦笋

- 使用技能3时，如果土豆骑士达到2名以上，就会额外消耗土豆骑士来提升伤害。（至少会保留1名土豆骑士。）

- 『源动力』叠到7层后会触发被动，自动释放技能3。
`,
        ...
        pros_cn: ["虽然是单体输出，但能借助总攻击打出范围伤害。", "可以通过追加效果削减敌人的DOWN值。"],
        ...
        cons_cn: ["想打到最大伤害时，需要额外调整甚至取消总攻击的发动时机。"],
    };
})();
