...
(function () {
    ...
    async function loadCharacterData() {
        ...
        const useKrFull = isKrLikeLang(lang) || STATE.spoiler;
    ...
    function getMindBaseProgress(inputs) {
        ...
        const current = hasCurrent ? inputs.mindBaseCurrent.filter(Boolean).length : 0;
        const target = hasTarget
            ? inputs.mindBaseTarget.filter(Boolean).length
            : (Array.isArray(inputs?.mindBase) ? inputs.mindBase.filter(Boolean).length : 0);
        return {
            current: Math.max(0, Math.min(12, current)),
    ...
    function renderSummary() {
        ...
        entries.forEach(([key, cnt]) => {
            ...
            let coveredOwned = STATE.inventory[key] || 0;
            if (key.startsWith('lv_limit')) {
                coveredOwned = limitSummary.visual[key] ?? 0;
            } else if (key.startsWith('lv_exp')) {
                coveredOwned = expSummary[key] ?? 0;
            } else if (key.startsWith('wp_limit')) {
                const mapKey = key.replace('wp_', 'lv_');
                coveredOwned = wpLimitSummary.visual[mapKey] ?? 0;
            } else if (key.startsWith('wp_exp')) {
                const mapKey = key.replace('wp_', 'lv_');
                coveredOwned = wpExpSummary[mapKey] ?? 0;
            } else if (key === 'skill_lv_1') {
                coveredOwned = skillSummary.visual.lv_limit1 ?? Math.min(cnt, STATE.inventory[key] || 0);
            } else if (key === 'skill_lv_2') {
                coveredOwned = skillSummary.visual.lv_limit2 ?? Math.min(cnt, STATE.inventory[key] || 0);
            } else if (key === 'skill_lv_3') {
                coveredOwned = skillSummary.visual.lv_limit3 ?? Math.min(cnt, STATE.inventory[key] || 0);
            } else {
                coveredOwned = Math.min(cnt, STATE.inventory[key] || 0);
            }
    ...
    function materialSortOrder(key) {
        ...
        // mind order follows the in-game inventory: shard, bottles, gems, bell
