...
/**
 * generate-schedule-pages.mjs
 *
 * Generates static SEO pages for the schedule app in each supported language.
 *
 * Usage:
 *   node scripts/seo/generate-schedule-pages.mjs
 *   node scripts/seo/generate-schedule-pages.mjs --check
 */
...
const OUTPUT_DIR = path.join(ROOT, 'pages', 'schedule');
const LANGS = ['kr', 'en', 'jp'];
const IMAGE_PATH = '/assets/img/home/SEO.png';

const seoMetaPath = path.join(ROOT, 'i18n', 'pages', 'schedule', 'seo-meta.json');
...
function ensureSeoMetaShape(meta) {
  for (const lang of LANGS) {
    ...
    if (typeof langMeta.title !== 'string' || !langMeta.title) {
      throw new Error(`i18n/pages/schedule/seo-meta.json missing valid title for ${lang}.`);
    }
    if (typeof langMeta.description !== 'string' || !langMeta.description) {
      throw new Error(`i18n/pages/schedule/seo-meta.json missing valid description for ${lang}.`);
    }
  }
}

function renderSchedulePage({ lang, title, description }) {
  const permalink = `/${lang}/schedule/`;
  const altKo = '/kr/schedule/';
  const altEn = '/en/schedule/';
  const altJp = '/jp/schedule/';
...
function buildExpectedFiles(seoMeta) {
  ...
  for (const lang of LANGS) {
    ...
    const content = normalizeNewline(
      renderSchedulePage({ lang, title, description })
    );
...
function runCheck(expectedFiles) {
  ...
  if (missing.length || changed.length || extra.length) {
    console.error('Schedule SEO pages are out of date.');
    if (missing.length) {
  ...
  console.log(`Schedule SEO pages are up to date (${expectedFiles.size} files).`);
}
...
function runGenerate(expectedFiles) {
  ...
  console.log(`Generated ${expectedFiles.size} schedule SEO files in ${toPosix(path.relative(ROOT, OUTPUT_DIR))}`);
}
