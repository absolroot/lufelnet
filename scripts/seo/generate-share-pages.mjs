...
const OUTPUT_DIR = path.join(ROOT, 'pages', 'share');
const LANGS = ['kr', 'en', 'jp', 'cn'];
const IMAGE_PATH = '/assets/img/home/SEO.png';
const seoMetaPath = path.join(ROOT, 'i18n', 'pages', 'share', 'seo-meta.json');
...
function ensureSeoMetaShape(meta) {
  for (const lang of LANGS) {
    ...
    if (typeof langMeta.title !== 'string' || !langMeta.title) {
      throw new Error(`i18n/pages/share/seo-meta.json missing valid title for ${lang}.`);
    }
    if (typeof langMeta.description !== 'string' || !langMeta.description) {
      throw new Error(`i18n/pages/share/seo-meta.json missing valid description for ${lang}.`);
    }
  }
}
...
function renderPage({ lang, title, description }) {
  return [
    ...
    `language: ${lang}`,
    `permalink: /${lang}/share/`,
    'alternate_urls:',
    '  ko: /kr/share/',
    '  en: /en/share/',
    '  jp: /jp/share/',
    '  zh-CN: /cn/share/',
    'share_use_last_tab: true',
    '---',
...
function runCheck(expectedFiles) {
  ...
  if (missing.length || changed.length || extra.length) {
    console.error('Share hub SEO pages are out of date.');
    if (missing.length) {
  ...
  console.log(`Share hub SEO pages are up to date (${expectedFiles.size} files).`);
}
...
function runGenerate(expectedFiles) {
  ...
  console.log(`Generated ${expectedFiles.size} share hub SEO files in ${toPosix(path.relative(ROOT, OUTPUT_DIR))}`);
}
