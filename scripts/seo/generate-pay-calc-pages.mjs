...
/**
 * generate-pay-calc-pages.mjs
 *
 * Generates static SEO pages for the pay-calc app.
 *
 * Usage:
 *   node scripts/seo/generate-pay-calc-pages.mjs
 *   node scripts/seo/generate-pay-calc-pages.mjs --check
 */
...
const OUTPUT_DIR = path.join(ROOT, 'pages', 'pay-calc');
const LANGS = ['kr'];
const IMAGE_PATH = '/assets/img/home/SEO.png';

const seoMetaPath = path.join(ROOT, 'i18n', 'pages', 'pay-calc', 'seo-meta.json');
...
function ensureSeoMetaShape(meta) {
  for (const lang of LANGS) {
    ...
    if (typeof langMeta.title !== 'string' || !langMeta.title) {
      throw new Error(`i18n/pages/pay-calc/seo-meta.json missing valid title for ${lang}.`);
    }
    if (typeof langMeta.description !== 'string' || !langMeta.description) {
      throw new Error(`i18n/pages/pay-calc/seo-meta.json missing valid description for ${lang}.`);
    }
  }
}

function renderPayCalcPage({ lang, title, description }) {
  const permalink = `/${lang}/pay-calc/`;
  const altKo = '/kr/pay-calc/';
...
function buildExpectedFiles(seoMeta) {
  ...
  for (const lang of LANGS) {
    ...
    const content = normalizeNewline(
      renderPayCalcPage({ lang, title, description })
    );
...
function runCheck(expectedFiles) {
  ...
  if (missing.length || changed.length || extra.length) {
    console.error('Pay-calc SEO pages are out of date.');
    if (missing.length) {
  ...
  console.log(`Pay-calc SEO pages are up to date (${expectedFiles.size} files).`);
}
...
function runGenerate(expectedFiles) {
  ...
  console.log(`Generated ${expectedFiles.size} pay-calc SEO files in ${toPosix(path.relative(ROOT, OUTPUT_DIR))}`);
}
