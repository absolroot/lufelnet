...
/**
 * generate-about-pages.mjs
 *
 * Generates static SEO pages for the about app in each supported language.
 *
 * Usage:
 *   node scripts/seo/generate-about-pages.mjs
 *   node scripts/seo/generate-about-pages.mjs --check
 */
...
const OUTPUT_DIR = path.join(ROOT, 'pages', 'about');
const LANGS = ['kr', 'en', 'jp'];
const IMAGE_PATH = '/assets/img/home/SEO.png';

const seoMetaPath = path.join(ROOT, 'i18n', 'pages', 'about', 'seo-meta.json');
...
function ensureSeoMetaShape(meta) {
  for (const lang of LANGS) {
    ...
    if (typeof langMeta.title !== 'string' || !langMeta.title) {
      throw new Error(`i18n/pages/about/seo-meta.json missing valid title for ${lang}.`);
    }
    if (typeof langMeta.description !== 'string' || !langMeta.description) {
      throw new Error(`i18n/pages/about/seo-meta.json missing valid description for ${lang}.`);
    }
  }
}

function renderAboutPage({ lang, title, description }) {
  const permalink = `/${lang}/about/`;
  const altKo = '/kr/about/';
  const altEn = '/en/about/';
  const altJp = '/jp/about/';
...
function buildExpectedFiles(seoMeta) {
  ...
  for (const lang of LANGS) {
    ...
    const content = normalizeNewline(
      renderAboutPage({ lang, title, description })
    );
...
function runCheck(expectedFiles) {
  ...
  if (missing.length || changed.length || extra.length) {
    console.error('About SEO pages are out of date.');
    if (missing.length) {
  ...
  console.log(`About SEO pages are up to date (${expectedFiles.size} files).`);
}
...
function runGenerate(expectedFiles) {
  ...
  console.log(`Generated ${expectedFiles.size} about SEO files in ${toPosix(path.relative(ROOT, OUTPUT_DIR))}`);
}
