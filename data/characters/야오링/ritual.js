window.ritualData = window.ritualData || {};
window.enCharacterRitualData = window.enCharacterRitualData || {};
window.jpCharacterRitualData = window.jpCharacterRitualData || {};
window.ritualData["야오링"] = {
  "name": "리 야오링",
  "r0": "유유한 망천수",
  "r0_detail": "자신의 턴 시작 시 속도 10포인트마다 『추억』 1포인트를 획득한다. 이 방식으로 턴마다 『추억』을 최대 18포인트 획득한다. 『추억』 40포인트 도달 시 자신은 모든 『추억』을 소모해 『맹파탕』을 획득한다. 스킬 시전 시 자신이 『맹파탕』을 보유하고 있으면 일부 스킬이 강화된다. 또한 1턴 동안 50%의 기본 확률로 스킬 메인 목표를 망각 상태에 빠뜨린다.",
  "r1": "돌이킬 수 없는 슬픔",
  "r1_detail": "전투 시작 시 『맹파탕』을 획득한다.",
  "r2": "적적한 다리",
  "r2_detail": "『사자놀이』, 『전투 기술』의 스킬 레벨이 3레벨 증가하고, 최대 15레벨까지 증가한다.",
  "r3": "잔잔한 이별",
  "r3_detail": "목표물 1명에게 디버프를 1개 부여할 때마다 리 야오링의 공격력이 2턴 동안 10% 증가한다(5회 중첩 가능).",
  "r4": "진한 맹파탕",
  "r4_detail": "『나룻배 사공』, 『피안의 덤불』의 스킬 레벨이 3레벨 증가하고, 최대 15레벨까지 증가한다.",
  "r5": "안개같은 지난 일",
  "r5_detail": "HIGHLIGHT 효과 증가: 적이 받는 대미지가 20% 추가 증가하고, 모든 적이 주원 효과를 1개 획득한다.",
  "r6": "붉은 만주",
  "r6_detail": "스킬을 시전해 『맹파탕』을 소모한 후 1턴 동안 모든 동료의 주원 대미지가 20% 증가하며, 60%의 고정 확률로 『맹파탕』이 반환된다. 해당 효과의 쿨타임은 1턴이다."
};
window.enCharacterRitualData["야오링"] = {
  "name": "Yaoling Li",
  "r0": "Goddess of Oblivion",
  "r0_detail": "On Yaoling's action, gain 1 Memory stack for every 10 points of Speed (up to 18 stacks per turn). When Memory reaches 40 stacks, spend all stacks to gain 1 Meng Po Soup stack.\nWhen using a skill, spend 1 Meng Po Soup stack for a 50% chance to inflict Forget on 1 foe for 1 turn, and enhance effects of Flowers of Naihe and Lion Dance of Oblivion.",
  "r1": "Road to Rebirth",
  "r1_detail": "At the start of battle, gain 1 Meng Po Soup stack.",
  "r2": "Soul Reaper",
  "r2_detail": "Increase Attack by 10% for each debuff inflicted on foes for 2 turns. Stacks up to 5 times.",
  "r3": "Beyond the Bend",
  "r3_detail": "Increase the skill levels of Lion Dance of Oblivion and Thief Tactics by 3.",
  "r4": "Training Results",
  "r4_detail": "Highlight Enhanced: Increase damage taken effect by 20%. Inflict Curse on all foes for 2 turns.",
  "r5": "Meng Po's Medicine",
  "r5_detail": "Increase the skill levels of Underworld Ferry and Flowers of Naihe by 3.",
  "r6": "Wisps of Crimson",
  "r6_detail": "When spending Meng Po Soup, increase party's Curse damage by 20% for 1 turn. Also, 60% chance to gain 1 Meng Po Soup stack. This effect won't activate again on the next turn."
};
window.jpCharacterRitualData["야오링"] = {
  "name": "李 瑤鈴",
  "r0": "忘川の女神",
  "r0_detail": "自身の行動時、自身の速さ１０ごとに『回想』を１つ獲得する。『回想』は、この効果で１ターンに最大１８まで獲得できる。『回想』が４０以上になった時、それらをすべて消費して『孟婆湯』を１つ獲得する。\nスキルを使用した時、『孟婆湯』を１つ消費して選択した敵を１ターンの間、５０%の確率で忘却状態にし、『黄泉路に咲く花』『泉下の獅子』を強化する。",
  "r1": "転生の導き手",
  "r1_detail": "戦闘開始時、『孟婆湯』を１つ獲得する。",
  "r2": "魂魄の捕縛",
  "r2_detail": "敵に弱体効果を付与するごとに、２ターンの間、自身の攻撃力が１０%上昇する。この効果は最大５回まで累積できる。",
  "r3": "奈何橋のたもとで",
  "r3_detail": "『泉下の獅子』『怪盗戦技』のスキルレベルが３上昇する。",
  "r4": "修行の成果",
  "r4_detail": "ハイライト能力追加：被ダメージ上昇効果を２０%引き上げ、さらに２ターンの間、敵全体を呪印状態にする。",
  "r5": "モウバの薬湯",
  "r5_detail": "『黄泉の渡し』『黄泉路に咲く花』のスキルレベルが３上昇する。",
  "r6": "曼珠沙華の紅",
  "r6_detail": "『孟婆湯』を消費した時、１ターンの間、味方の呪怨属性による与ダメージが２０%上昇する。また、６０%の確率で『孟婆湯』を１つ獲得する。この効果は発動した次のターンは発動しない。"
};
window.cnCharacterRitualData = window.cnCharacterRitualData || {};

window.cnCharacterRitualData["야오링"] = {
  "name": "李瑶铃",
  "r0": "忘川水悠悠",
  "r0_detail": "自身回合开始时，每有10点速度，就获得1点『回忆』，通过这种方式每回合最多获得18点『回忆』。『回忆』到达40点时会消耗所有『回忆』使自身获得『孟婆汤』。释放技能时，若自己持有『孟婆汤』则使部分技能获得强化。并有50%的基础概率使技能主目标陷入遗忘状态，效果持续1回合。",
  "r1": "魂断无回头",
  "r1_detail": "战斗开始时，获得『孟婆汤』。",
  "r2": "奈何桥寂寂",
  "r2_detail": "『顽狮闹』、『战斗技巧』的技能等级提升3级，至多提升至15级。",
  "r3": "离别惹涟漪",
  "r3_detail": "每对1个目标造成1次减益效果，就使李瑶铃的攻击力提升10%，持续2回合，上限5层。",
  "r4": "孟婆汤浓浓",
  "r4_detail": "『摆渡者』、『彼岸之丛』的技能等级提升3级，至多提升至15级。",
  "r5": "往事如烟空",
  "r5_detail": "HIGHLIGHT效果提升：使敌人受到的伤害额外提升20%，并使所有敌人获得一个诅咒效果。",
  "r6": "曼珠如烟红",
  "r6_detail": "释放技能消耗『孟婆汤』后，所有同伴诅咒伤害提升20%，持续1回合，并有60%固定概率返还『孟婆汤』，该效果有1回合冷却时间。"
};
