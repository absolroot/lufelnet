 ...
 *   node scripts/seo/generate-revelation-setting-pages.mjs --check
 */
...
function ensureSeoMetaShape(meta) {
  for (const lang of LANGS) {
    ...
    if (typeof langMeta.title !== 'string' || !langMeta.title) {
      throw new Error(`i18n/pages/revelation-setting/seo-meta.json missing valid title for ${lang}.`);
    }
    if (typeof langMeta.description !== 'string' || !langMeta.description) {
      throw new Error(`i18n/pages/revelation-setting/seo-meta.json missing valid description for ${lang}.`);
    }
  }
}
...
function runCheck(expectedFiles) {
  ...
  if (missing.length || changed.length || extra.length) {
    console.error('Revelation Setting SEO pages are out of date.');
    if (missing.length) {
  ...
  console.log(`Revelation Setting SEO pages are up to date (${expectedFiles.size} files).`);
}
...
function runGenerate(expectedFiles) {
  ...
  console.log(`Generated ${expectedFiles.size} revelation-setting SEO files in ${toPosix(path.relative(ROOT, OUTPUT_DIR))}`);
}
