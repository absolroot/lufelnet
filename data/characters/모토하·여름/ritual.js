window.ritualData = window.ritualData || {};
window.enCharacterRitualData = window.enCharacterRitualData || {};
window.jpCharacterRitualData = window.jpCharacterRitualData || {};
window.cnCharacterRitualData = window.cnCharacterRitualData || {};

window.ritualData["모토하·여름"] = {
  "name": "아라이 모토하·여름",
  "r0": "햇빛과 푸른 바다",
  "r0_detail": "자신이 치료 효과를 받은 후 2턴 동안 대미지가 25% 증가한다. 누적 5회 치료 효과를 받은 후 자신이 『활기찬 여름』(축복의 치료 효과를 받을 시 턴마다 1회 치료로 간주)을 획득한다.\n『파도 추격자』 스킬 사용 시 『활기찬 여름』을 소모하여 『파도 추격자』가 스킬 쿨타임 상태에 진입하지 않게 한다. 이후 다시 『활기찬 여름』을 획득하면 1턴의 쿨타임이 주어진다.\n자신이 스킬로 생명 소모 시 자신이 사망 상태에 빠지지 않는다.",
  "r1": "발끝에 닿는 파도",
  "r1_detail": "『한여름』 상태일 때 크리티컬 확률 10%, 크리티컬 효과가 25% 추가 증가한다. 턴마다 시작 시 축복을 1턴 획득하며, 받는 치료 횟수가 1회 증가한다.",
  "r2": "쏟아지는 금색 빛",
  "r2_detail": "『여유로운 휴가』, 『파도 추격자』의 스킬 레벨이 3레벨 증가하고, 최대 15레벨까지 증가한다.",
  "r3": "먼 곳의 흔들리는 범선",
  "r3_detail": "자신이 치료 효과를 받을 때, 2턴 동안 20%의 크리티컬 효과 보너스를 획득한다(2회 중첩 가능). 자신의 최대 생명이 1,200포인트 증가한다.",
  "r4": "바닷바람 속의 짠 내음",
  "r4_detail": "『여름날 마법』, 『전투 기술』의 스킬 레벨이 3레벨 증가하고, 최대 15레벨까지 증가한다.",
  "r5": "높은 하늘 위에서 웃는 햇살",
  "r5_detail": "HIGHLIGHT 효과 증가, 자신의 다음 『파도 추격자』 대미지가 80% 증가한다.",
  "r6": "손을 잡고 여름 산책",
  "r6_detail": "『활기찬 여름』 효과는 스킬 『파도 추격자』 사용 후 더는 소모되지 않는다.\n『한여름』 효과가 『여름날의 태양』으로 강화된다. 『여름날의 태양』 상태에서 『여름날 마법』이 추가 대미지를 3회 주며, 『여유로운 휴가』가 추가로 자신의 최대 생명 20%의 HP를 회복한다. 『파도 추격자』 스킬 배율이 50% 증가한다."
};

window.enCharacterRitualData["모토하·여름"] = {
  "name": "Tropical Motoha",
  "r0": "Summer Starts Here!",
  "r0_detail": "When receiving healing, increase damage by 25% for 2 turns. After receiving healing a total of 5 times, gain Tropical Heart (healing received from Blessing counts once per turn).\nWhen using Surf 'n' Shine while Tropical Heart is active, spend Tropical Heart to remove the cooldown time from Surf 'n' Shine. Afterwards, there is a 1-turn cooldown until Tropical Heart can be gained again.\nWhen Tropical Motoha spends HP to use a skill, she will not be KO'd.",
  "r1": "Exciting Summer!",
  "r1_detail": "When Summer Hype is active, increase critical rate by 10% and critical damage by 25%.\nOn Tropical Motoha's action, increase Tropical Heart's healing counter by 1 and gain 1 Blessing.",
  "r2": "Stay Cool!",
  "r2_detail": "When Tropical Motoha recieves healing, increase critical damage by 20% for 2 turns. Stacks up to 2 times.\nDuring battle, increase her max HP by 1200.",
  "r3": "Catch the Wave!",
  "r3_detail": "Increase the skill levels of Summer 'Splosion and Surf 'n' Shine by 3.",
  "r4": "The Fun Never Ends!",
  "r4_detail": "Highlight Enhanced: After using a Highlight, increase the next Surf 'n' Shine's damage by 80%.",
  "r5": "Another Perfect Day!",
  "r5_detail": "Increase the skill levels of Blue Sunrise and Thief Tactics by 3.",
  "r6": "Everlasting Summer!",
  "r6_detail": "Tropical Heart will not be spent after using Surf 'n' Shine.\nSummer Hype is enhanced to Summer Blowout. When Summer Blowout is active, the bonus damage of Blue Sunrise activates 3 times. Summer 'Splosion recovers 20% more of Tropical Motoha's max HP. Increase the skill multiplier of Surf 'n' Shine by 50%."
};

window.jpCharacterRitualData["모토하·여름"] = {
  "name": "素羽・トロピカル",
  "r0": "夏はこれから！",
  "r0_detail": "自身が回復を受けた時、２ターンの間、与ダメージが２５％上昇する。さらに回復を累計で５回受けると『トロピカル・ハート』を獲得する（祝印による回復は、毎ターン１回分としてカウントする）。\n『サーフ＆シャイン』を使用した時、『トロピカル・ハート』状態ならば、それを消費することで『サーフ＆シャイン』のクールタイムが発生しなくなる。その後『トロピカル・ハート』を再び獲得するまでに、１ターンのクールタイムが発生する。\nまた自身がスキルでＨＰを消費しても戦闘不能にならない。",
  "r1": "わくわくサマー！",
  "r1_detail": "『真夏のテンション』状態の時、クリティカル率が１０%上昇し、クリティカルダメージが２５%上昇する。\nまた行動時、『トロピカル・ハート』の、回復を受けた回数が１回分増加し、さらに祝印を１つ獲得する。",
  "r2": "夏バテ防止！",
  "r2_detail": "自身が回復を受けた時、２ターンの間、クリティカルダメージが２０%上昇する。最大２つまで累積できる。\nさらに戦闘中、自身の最大ＨＰが１２００上昇する。",
  "r3": "良い波来てるよ！",
  "r3_detail": "『ハッピーサマーボム』『サーフ＆シャイン』のスキルレベルが３上昇する。",
  "r4": "遊び足りない！",
  "r4_detail": "ハイライト能力追加：ハイライト使用後、次の『サーフ＆シャイン』の与ダメージが８０%上昇する。",
  "r5": "今日も快晴！",
  "r5_detail": "『ブルーサンライズ』『怪盗戦技』のスキルレベルが３上昇する。",
  "r6": "夏は終わらない！",
  "r6_detail": "『トロピカル・ハート』が『サーフ＆シャイン』使用後に消費されなくなる。\n『真夏のテンション』が『真夏のハイテンション』に強化される。\n『真夏のハイテンション』状態の時、『ブルーサンライズ』の追加ダメージが３回発動する。『ハッピーサマーボム』使用時、追加で自身の最大ＨＰの２０%を回復する。また、『サーフ＆シャイン』のスキル倍率が５０%上昇する。"
};

window.cnCharacterRitualData["모토하·여름"] = {
  "name": "新井素羽·夏日",
  "r0": "阳光与蔚蓝海面",
  "r0_detail": "自身受到治疗效果后，伤害提升25%，持续2回合。累计受到5次治疗效果后，自身获得『跃动一夏』（受到祝福的治疗效果时每回合视为1次治疗）。\n使用技能『海浪追逐者』时，消耗『跃动一夏』，使『海浪追逐者』不进入技能冷却状态，此后再次获得『跃动一夏』有1回合的冷却时间。\n自身通过技能消耗生命值时不会使自身陷入死亡状态。",
  "r1": "浪花轻吻脚尖",
  "r1_detail": "『盛夏』状态下额外增加10%暴击率，25%暴击效果。每回合开始时获得1层祝福并且受到治疗次数增加1次。",
  "r2": "洒落的金色光辉",
  "r2_detail": "『悠闲假期』，『海浪追逐者』的技能等级提升3级，至多提升至15级。",
  "r3": "远方的帆船摇曳",
  "r3_detail": "自身受到治疗效果时，获得20%暴击效果加成，持续2回合，上限2层。自身最大生命值提升1200点。",
  "r4": "海风中的咸香",
  "r4_detail": "『夏日清凉魔法』，『战斗技巧』的技能等级提升3级，至多提升至15级。",
  "r5": "艳阳于高空轻笑",
  "r5_detail": "HIGHLIGHT效果提升，使自身下一次『海浪追逐者』伤害提高80%。",
  "r6": "牵手在夏日漫步",
  "r6_detail": "『跃动一夏』效果在使用技能『海浪追逐者』后不再消耗。\n『盛夏』效果强化为『真夏艳阳』。『真夏艳阳』状态下，『夏日清凉魔法』的追加伤害会触发3次；『悠闲假期』额外恢复自身最大生命值的20%血量；『海浪追逐者』技能倍率提升50%。"
};

