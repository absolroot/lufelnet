...
const OUTPUT_DIR = path.join(ROOT, 'pages', '2.5-anniversary');
const LANGS = ['kr', 'en', 'jp', 'cn'];
const IMAGE_PATH = '/assets/img/home/SEO.png';
const seoMetaPath = path.join(ROOT, 'i18n', 'pages', 'two-half-anniversary', 'seo-meta.json');
...
function ensureSeoMetaShape(meta) {
  for (const lang of LANGS) {
    ...
    if (typeof langMeta.title !== 'string' || !langMeta.title) {
      throw new Error(`i18n/pages/two-half-anniversary/seo-meta.json missing valid title for ${lang}.`);
    }
    if (typeof langMeta.description !== 'string' || !langMeta.description) {
      throw new Error(`i18n/pages/two-half-anniversary/seo-meta.json missing valid description for ${lang}.`);
    }
  }
}
...
function renderPage({ lang, title, description }) {
  return [
    ...
    `language: ${lang}`,
    `permalink: /${lang}/2.5-anniversary/`,
    'alternate_urls:',
    '  ko: /kr/2.5-anniversary/',
    '  en: /en/2.5-anniversary/',
    '  jp: /jp/2.5-anniversary/',
    '  cn: /cn/2.5-anniversary/',
    '---',
    '{% include two-half-anniversary-body.html %}',
    ''
  ].join('\n');
}
...
function runCheck(expectedFiles) {
  ...
  if (missing.length || changed.length || extra.length) {
    console.error('Two-half anniversary SEO pages are out of date.');
    if (missing.length) {
  ...
  console.log(`Two-half anniversary SEO pages are up to date (${expectedFiles.size} files).`);
}
...
function runGenerate(expectedFiles) {
  ...
  console.log(`Generated ${expectedFiles.size} two-half anniversary SEO files in ${toPosix(path.relative(ROOT, OUTPUT_DIR))}`);
}
