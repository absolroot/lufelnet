...
"미츠루": { name_en: "Kirijo Mitsuru", name_jp: "きりじょう みつる", codename: "MITSURU",
        review: `
포지션은 '반항'이지만 **'서포터'**의 역할이 강한 P3 콜라보 캐릭터로 기존에 지배 포지션이었지만 아군의 서포트를 담당했던 아케치의 반항 버전이다.

- **스킬2**로 방어력 감소와 받는 대미지 증가를 적 1명에게 영구 지속시키고, **스킬1**로 **『서리 결속』3스택**과 함께 방어력 감소를 부여한다.

- **스킬3**은 일정 수치(3%)의 해명 괴도의 속성 증가를 포함하고 있으며, 높은 수치의 빙결 속성 대미지를 준다. 크리티컬 확률 15%의 효과는 의식 1부터 유효하다.
...
**『서리 결속』의 중첩 수**는 **의식0/1/6**에 각각 **3/5/6**개를 유지할 수 있다.

의식6에는 추가 피해 대미지가 2배가 더해지므로 아군이 공격할 때 **스킬 계수**에 **66%/110%/292.8%**를 더하는 형태로 의식 6의 계수 격차가 현저히 높다. 

의식6 반항 괴도 기준 약 1턴에 400~500%의 스킬 계수 대미지를 부여하므로 450%를 기준 삼을 경우 14.6%/24.4%/65.1%의 최종 대미지 증가 효율을 얻는다. 
...
<table border="1">
  ...
  <tbody>
    <tr>
      <td>의식0 (LV10+5) - 3스택</td>
      <td>81.8%</td>
      <td>14.4%</td>
      <td>66.0%</td>
    </tr>
    <tr>
      <td>의식1 (LV10+5) - 5스택</td>
      <td>107.6%</td>
      <td>24.0%</td>
      <td>110.0%</td>
    </tr>
    <tr>
      <td>의식6 (LV13+5) - 6스택</td>
      <td>127.2%</td>
      <td>28.8%</td>
      <td>292.8%</td>
    </tr>
...
지속적으로 공격을 수행하므로 방어력 감소 수치가 부족할 경우에는 [주권]+[여정] 계시를 고려해볼 수 있다.

**※ 중국 서버에 출시된지 얼마되지 않아 수치 계산 상 오류가 있을 수 있습니다.**
        `,
