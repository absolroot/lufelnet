...
/**
 * generate-astrolabe-pages.mjs
 *
 * Generates static SEO pages for the astrolabe app in each supported language.
 *
 * Usage:
 *   node scripts/seo/generate-astrolabe-pages.mjs
 *   node scripts/seo/generate-astrolabe-pages.mjs --check
 */
...
const OUTPUT_DIR = path.join(ROOT, 'pages', 'astrolabe');
const LANGS = ['kr', 'en', 'jp', 'cn'];
const IMAGE_PATH = '/assets/img/home/SEO.png';

const seoMetaPath = path.join(ROOT, 'i18n', 'pages', 'astrolabe', 'seo-meta.json');
...
function ensureSeoMetaShape(meta) {
  for (const lang of LANGS) {
    ...
    if (typeof langMeta.title !== 'string' || !langMeta.title) {
      throw new Error(`i18n/pages/astrolabe/seo-meta.json missing valid title for ${lang}.`);
    }
    if (typeof langMeta.description !== 'string' || !langMeta.description) {
      throw new Error(`i18n/pages/astrolabe/seo-meta.json missing valid description for ${lang}.`);
    }
  }
}

function renderAstrolabePage({ lang, title, description }) {
  const permalink = `/${lang}/astrolabe/`;
  const altKo = '/kr/astrolabe/';
  const altEn = '/en/astrolabe/';
  const altJp = '/jp/astrolabe/';
  const altZhCn = '/cn/astrolabe/';
...
function buildExpectedFiles(seoMeta) {
  ...
  for (const lang of LANGS) {
    ...
    const content = normalizeNewline(
      renderAstrolabePage({ lang, title, description })
    );
...
function runCheck(expectedFiles) {
  ...
  if (missing.length || changed.length || extra.length) {
    console.error('Astrolabe SEO pages are out of date.');
    if (missing.length) {
  ...
  console.log(`Astrolabe SEO pages are up to date (${expectedFiles.size} files).`);
}
...
function runGenerate(expectedFiles) {
  ...
  console.log(`Generated ${expectedFiles.size} astrolabe SEO files in ${toPosix(path.relative(ROOT, OUTPUT_DIR))}`);
}
