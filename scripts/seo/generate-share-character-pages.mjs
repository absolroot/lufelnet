...
const OUTPUT_DIR = path.join(ROOT, 'pages', 'share-character');
const LANGS = ['kr', 'en', 'jp', 'cn'];
const IMAGE_PATH = '/assets/img/home/SEO.png';
const seoMetaPath = path.join(ROOT, 'i18n', 'pages', 'share-character', 'seo-meta.json');
...
function ensureSeoMetaShape(meta) {
  for (const lang of LANGS) {
    ...
    if (typeof langMeta.title !== 'string' || !langMeta.title) {
      throw new Error(`i18n/pages/share-character/seo-meta.json missing valid title for ${lang}.`);
    }
    if (typeof langMeta.description !== 'string' || !langMeta.description) {
      throw new Error(`i18n/pages/share-character/seo-meta.json missing valid description for ${lang}.`);
    }
  }
}
...
function renderPage({ lang, title, description }) {
  return [
    ...
    `language: ${lang}`,
    `permalink: /${lang}/share/character/`,
    'alternate_urls:',
    '  ko: /kr/share/character/',
    '  en: /en/share/character/',
    '  jp: /jp/share/character/',
    '  zh-CN: /cn/share/character/',
    '---',
    '{% include share-character-body.html %}',
    ''
  ].join('\n');
}
...
function buildExpectedFiles(seoMeta) {
  ...
  for (const lang of LANGS) {
    ...
    const content = normalizeNewline(renderPage({
      lang,
...
function runCheck(expectedFiles) {
  ...
  const existingFiles = listExistingFiles(OUTPUT_DIR).map((fullPath) => toPosix(path.relative(ROOT, fullPath)));
  const extra = existingFiles.filter((relativePath) => !expectedSet.has(relativePath));

  if (missing.length || changed.length || extra.length) {
    console.error('Share Character SEO pages are out of date.');
    if (missing.length) {
  ...
  console.log(`Share Character SEO pages are up to date (${expectedFiles.size} files).`);
}
...
function runGenerate(expectedFiles) {
  ...
  console.log(`Generated ${expectedFiles.size} share-character SEO files in ${toPosix(path.relative(ROOT, OUTPUT_DIR))}`);
}
