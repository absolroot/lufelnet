(function () {
    window.characterReview = window.characterReview || {};
    window.characterReview["마유미"] = { name_en: "Mayumi Hashimoto", name_jp: "橋本 麻由美", codename: "TURBO",
        review: `관통 속성에 특화된 물리 속성의 버퍼다. 모든 괴도 행동 종료시마다 시속 스택을 획득한다.

**시속 스택 누적 시 추가 턴으로 진입**할 수 있으며, 추가 턴에 진입 후 사용하는 버프는 효과가 강화되어 1스킬은 확정 크리티컬 공격, 2스킬은 실드 부여, 3스킬은 관통 효과 증폭을 획득한다.

의식 1 이상이 될 경우 시속 획득이 원할해져, 2턴에 한 번씩 추가 턴으로 진입할 수 있게 된다.

<br>

관통 메커니즘이 변경되기 이전에 출시됐지만, **관통 속성이 큰 상향을 받으면서 가치가 폭발적으로 상승했다**.

단순 관통 수치 뿐 아니라 공격력과 대미지 보너스 버프, HIGHLIGHT까지 유용한 버프를 제공해서 **범용적이고 강력한 성능**을 발휘한다.

**제대로 활용하기 위해서는 속도 스탯을 상당히 높게** 올려야하는데 그 난이도가 쉽지 않다. 

또한 스킬1의 크리티컬을 통해 DOWN 수치를 감소시키는 효과를 활용하려면 크리티컬 확률 또한 신경써주는 것이 중요하다.

특이사항으로 하이라이트는 3턴으로 명시돼있지만 실질적으로는 3.1턴 지속시간으로, HL을 사용하는 시점에 행동을 대기하던 캐릭터는 해당 턴을 포함 총 4턴간 버프가 지속된다.`,
        review_en: `A Physical-support buffer specialized in Pierce. She gains Velocity stacks whenever any Phantom Thief finishes their action.

When enough Velocity stacks are accumulated, she enters an Extra Action. After entering an Extra Action, her buffs become enhanced: Skill 1 gains guaranteed Critical hits, Skill 2 grants a shield, and Skill 3 provides amplified Piercing effects.

With Resonance 1 or higher, Velocity generation becomes smoother, enabling her to enter an Extra Action roughly every two turns.

Although she was released before the Pierce mechanic rework, the attribute received significant buffs afterward, causing her value to skyrocket.

Beyond Pierce itself, she provides highly useful buffs such as Attack, Damage Bonus, and even HIGHLIGHT, giving her **excellent versatility and strong overall performance**.

To make full use of her kit, **her Speed stat must be raised significantly**, which can be challenging to achieve. 

Additionally, to utilize the effect of reducing DOWN values through Skill 1's Critical, it is important to pay attention to Critical Rate as well.

As a special note, the Highlight effect is specified as lasting 3 turns, but its actual duration is 3.1 turns. Characters who were waiting to act when the Highlight was used will have the buff last for a total of 4 turns, including that turn.
`,
        review_jp: `貫通属性に特化した物理属性のサポートバッファー。怪盗が行動を終えるたびに「時速」スタックを獲得する。

時速スタックが一定量溜まると追加行動に入ることができ、追加行動後に使用するバフは強化される。スキル1は確定クリティカル、スキル2はシールド付与、スキル3は貫通効果の増幅を得る。

共鳴1以上になると時速獲得がスムーズになり、2ターンに1回ほど追加行動へ入ることが可能になる。

貫通メカニクスが改修される前に実装されたが、その後**貫通属性が大幅に強化されたことで評価が爆発的に向上**した。

貫通だけでなく、攻撃力バフ・ダメージボーナス・HIGHLIGHTなど有用なバフも揃っており、**汎用性が高く非常に強力な性能**を発揮する。

真価を発揮するには**速度ステータスを大幅に盛る必要があり**, そのハードルは決して低くない。

また、スキル1のクリティカルによるDOWN値減少効果を活用するには、クリティカル率にも注意を払う必要がある。

特徴的には、ハイライトは3ターンと明示されていますが、実際は3.1ターンの持続時間で、ハイライトを使用したターンに行動を待機していたキャラクターは、そのターンを含めて合計4ターン間、バフが継続されます。
`,
        review_cn: `是专精穿透属性的物理辅助增益手。每当有怪盗结束行动时，她都会获得一层“时速”层数。

当时速层数累积到一定程度后，她就能进入额外回合；而在额外回合中释放的增益会获得强化，技能1会变成必定暴击攻击，技能2会附带护盾，技能3则会进一步强化穿透效果。

达到意识1以后，她获取时速的效率会明显提高，基本能做到每2回合进入一次额外回合。

<br>

虽然她是在穿透机制改版之前上线的角色，但**随着穿透属性后来被大幅加强，她的价值也迎来了明显上升**。

她不仅能提供穿透，还能带来攻击力、伤害加成，甚至连HIGHLIGHT都能提供很实用的增益，因此整体表现**既泛用又强力**。

不过若想真正发挥强度，**必须把速度属性堆到相当高的水平**，而这本身并不轻松。

此外，如果想利用技能1通过暴击来削减DOWN值，也需要额外兼顾暴击率。

比较特别的一点是，她的HIGHLIGHT虽然写着持续3回合，但实际更接近3.1回合，因此在HIGHLIGHT发动当回合还没行动的角色，实际会把这个增益带满4个行动回合。`,
        pros: ["높은 수치의 관통 효과를 제공한다.", "추가 턴 진입이 가능하다."],
        pros_en: [
            "Provides high Pierce values.",
            "Can enter an Extra Action."
          ],
        pros_jp: [
            "高い貫通効果を付与できる。",
            "追加行動へ入ることができる。"
          ],
        pros_cn: ["能够提供高额穿透效果。", "可以进入额外回合。"],
        cons: ["권장 육성 스탯 달성이 어렵다."],
        cons_en: ["Reaching the recommended Speed stat is difficult."],
        cons_jp: ["推奨される速度ステータスを確保するのが難しい。"],
        cons_cn: ["较难达到她推荐的速度养成线。"],
    };
})();

