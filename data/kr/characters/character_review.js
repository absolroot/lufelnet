...
`,
    review_jp: `
Ver2.0で登場したリコは、**ストーリーを進めることで加入する怪盗団メンバー**であり、キャラクター契約からは解放状況に関係なく即座に獲得できます。

敵に**マーク**を付与し、通常攻撃または弱点攻撃でマークを解除すると、**ガード値を追加で減少させてDOWNを発生**させることができます。

**弱点以外の攻撃でもDOWNを誘発できる**ため、**ONE MOREや総攻撃**を積極的に活用できます。

パッシブも**ONE MOREと総攻撃を強化**する構成となっており、ベルベットルームのミッション攻略に大きく貢献します。
...
自身のHPを消費してスキルを使用し、**核心となるスキル3**は**モトハがHPを回復するとクールタイムが短縮される効果**を持つため、**宮下 美波**や**野毛 朋子・夏**といったヒーラーとの編成が前提となる。
