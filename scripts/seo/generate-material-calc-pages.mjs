...
/**
 * generate-material-calc-pages.mjs
 *
 * Generates static SEO pages for the material-calc app in each supported language.
 *
 * Usage:
 *   node scripts/seo/generate-material-calc-pages.mjs
 *   node scripts/seo/generate-material-calc-pages.mjs --check
 */
...
const OUTPUT_DIR = path.join(ROOT, 'pages', 'material-calc');
const LANGS = ['kr', 'en', 'jp'];
const IMAGE_PATH = '/assets/img/home/SEO.png';

const seoMetaPath = path.join(ROOT, 'i18n', 'pages', 'material-calc', 'seo-meta.json');
...
function ensureSeoMetaShape(meta) {
  for (const lang of LANGS) {
    ...
    if (typeof langMeta.title !== 'string' || !langMeta.title) {
      throw new Error(`i18n/pages/material-calc/seo-meta.json missing valid title for ${lang}.`);
    }
    if (typeof langMeta.description !== 'string' || !langMeta.description) {
      throw new Error(`i18n/pages/material-calc/seo-meta.json missing valid description for ${lang}.`);
    }
  }
}

function renderMaterialCalcPage({ lang, title, description }) {
  const permalink = `/${lang}/material-calc/`;
  const altKo = '/kr/material-calc/';
  const altEn = '/en/material-calc/';
  const altJp = '/jp/material-calc/';
  ...
  return [
    ...
    `  jp: ${altJp}`,
    '---',
    '{% include material-calc-body.html %}',
    ''
  ].join('\n');
}
...
function buildExpectedFiles(seoMeta) {
  ...
  for (const lang of LANGS) {
    ...
    const content = normalizeNewline(
      renderMaterialCalcPage({ lang, title, description })
    );
...
function runCheck(expectedFiles) {
  ...
  if (missing.length || changed.length || extra.length) {
    console.error('Material-calc SEO pages are out of date.');
    if (missing.length) {
  ...
  console.log(`Material-calc SEO pages are up to date (${expectedFiles.size} files).`);
}
...
function runGenerate(expectedFiles) {
  ...
  console.log(`Generated ${expectedFiles.size} material-calc SEO files in ${toPosix(path.relative(ROOT, OUTPUT_DIR))}`);
}
