...
Although greatly influenced by SP recovery, the character can easily reach **283.9% SP recovery** even with just the base **188.5%** SP recovery, the **SP Recovery main stat of Revelation Card Sky (90.4%)**, and Thieves Den’s 5%.  
...
- スキル2スタックごとのダメージ増加計算式：スキル係数 × (100% + 10% × スタック数)  `,
        review_cn: `
在『海之领域』内，她会在友方回合结束时不断发动**每次攻击都会继续变强的追加效果**，持续压制敌人。

由于追加效果重复次数越多，精力值消耗也会越来越高，因此她的机制核心是**通过技能2打出高额伤害并重置层数**。

**友方回合开始时会回复15点精力值**，**自身回合开始时也会回复10点精力值**。这两个数值都会受到精力回复属性影响，因此如何尽量触发更多追加效果，就是这个角色的关键。

<br>

她确实非常依赖精力回复，但只靠**基础188.5%**精力回复、启示卡主词条提供的**90.4%精力回复**，再加上我的宫殿的5%，就能相对轻松地凑出**283.9%精力回复**。

如果达到意识6，还会额外获得250点精力值，因此就不再需要额外堆精力回复。（在280%精力回复以内都会被被动转化，不会产生攻击力亏损。）

按照推荐循环，所需的精力回复量如下。这里没有计算长尾爱歌（ANGE）带来的额外回合次数。
...
- 追加效果每层5%伤害提升的计算方式：技能系数 ×（100% + 5% × 层数）

- 技能2每层10%伤害提升的计算方式：技能系数 ×（100% + 10% × 层数）`,
        ...
        pros_cn: ["自动追加攻击会不断重复，几乎所有内容里都很好用。", "拥有延长增益1回合这一非常强力的效果。"],
        cons: ["SP 수치를 신경써야한다."],
        cons_en: ["Need to pay attention to SP values."],
        cons_jp: ["SPの数値に注意する必要があります。"],
        cons_cn: ["需要仔细管理精力回复与精力值。"],
    };
})();
