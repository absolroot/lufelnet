...
function buildExpectedFiles(wonderData, seoMeta, slugMap) {
  ...
  for (const krName of optionNames) {
    ...
    addRedirectStub(`/wonder-weapon/${slug}/`, canonicalKoPath, buildDetailLangTargets('wonder-weapon', slug));

    for (const alias of aliasSlugs) {
      addRedirectStub(`/wonder-weapon/${alias}/`, canonicalKoPath, buildDetailLangTargets('wonder-weapon', slug));
