...
/**
 * generate-defense-calc-pages.mjs
 *
 * Generates static SEO pages for the defense-calc app in each supported language.
 *
 * Usage:
 *   node scripts/seo/generate-defense-calc-pages.mjs
 *   node scripts/seo/generate-defense-calc-pages.mjs --check
 */
...
const OUTPUT_DIR = path.join(ROOT, 'pages', 'defense-calc');
const LANGS = ['kr', 'en', 'jp'];
const IMAGE_PATH = '/assets/img/home/SEO.png';

const seoMetaPath = path.join(ROOT, 'i18n', 'pages', 'defense-calc', 'seo-meta.json');
...
function ensureSeoMetaShape(meta) {
  for (const lang of LANGS) {
    ...
    if (typeof langMeta.title !== 'string' || !langMeta.title) {
      throw new Error(`i18n/pages/defense-calc/seo-meta.json missing valid title for ${lang}.`);
    }
    if (typeof langMeta.description !== 'string' || !langMeta.description) {
      throw new Error(`i18n/pages/defense-calc/seo-meta.json missing valid description for ${lang}.`);
    }
  }
}

function renderDefenseCalcPage({ lang, title, description }) {
  const permalink = `/${lang}/defense-calc/`;
  const altKo = '/kr/defense-calc/';
  const altEn = '/en/defense-calc/';
  const altJp = '/jp/defense-calc/';

  return [
    '---',
    'layout: default',
    'custom_css: []',
    'custom_js: [tooltip, character/spoiler-state, character/character-list, defense/defense-i18n]',
    'custom_data: [tooltip.js, calc/boss.js, calc/defense-data.js, wonder/weapons.js, wonder/skills.js,',
    ...
    `  jp: ${altJp}`,
    '---',
    '{% include defense-calc-body.html %}',
    ''
  ].join('\n');
}
...
function buildExpectedFiles(seoMeta) {
  ...
  for (const lang of LANGS) {
    ...
    const content = normalizeNewline(
      renderDefenseCalcPage({ lang, title, description })
    );
...
function runCheck(expectedFiles) {
  ...
  if (missing.length || changed.length || extra.length) {
    console.error('Defense-calc SEO pages are out of date.');
    if (missing.length) {
  ...
  console.log(`Defense-calc SEO pages are up to date (${expectedFiles.size} files).`);
}
...
function runGenerate(expectedFiles) {
  ...
  console.log(`Generated ${expectedFiles.size} defense-calc SEO files in ${toPosix(path.relative(ROOT, OUTPUT_DIR))}`);
}
