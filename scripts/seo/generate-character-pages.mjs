...
function renderCharacterListPage({ lang, title, description }) {
  return [
    ...
    "  'zh-CN': /cn/character/",
    '---',
    '{% include character-list-body.html %}',
    ''
  ].join('\n');
}
...
function buildExpectedFiles(allCharacters, characterData, seoMeta, slugMap) {
  ...
  for (const krName of characterNames) {
    ...
    addRedirectStub(`/character/${slug}/`, canonicalKoPath, buildDetailLangTargets('character', slug));

    for (const alias of aliasSlugs) {
      addRedirectStub(`/character/${alias}/`, canonicalKoPath, buildDetailLangTargets('character', slug));
