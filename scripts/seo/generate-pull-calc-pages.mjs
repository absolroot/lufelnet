...
/**
 * generate-pull-calc-pages.mjs
 *
 * Generates static SEO pages for the pull-calc app in each supported language.
 *
 * Usage:
 *   node scripts/seo/generate-pull-calc-pages.mjs
 *   node scripts/seo/generate-pull-calc-pages.mjs --check
 */
...
const OUTPUT_DIR = path.join(ROOT, 'pages', 'pull-calc');
const LANGS = ['kr', 'en', 'jp'];
const IMAGE_PATH = '/assets/img/home/SEO.png';

const seoMetaPath = path.join(ROOT, 'i18n', 'pages', 'pull-calc', 'seo-meta.json');
...
function ensureSeoMetaShape(meta) {
  for (const lang of LANGS) {
    ...
    if (typeof langMeta.title !== 'string' || !langMeta.title) {
      throw new Error(`i18n/pages/pull-calc/seo-meta.json missing valid title for ${lang}.`);
    }
    if (typeof langMeta.description !== 'string' || !langMeta.description) {
      throw new Error(`i18n/pages/pull-calc/seo-meta.json missing valid description for ${lang}.`);
    }
  }
}

function renderPullCalcPage({ lang, title, description }) {
  const permalink = `/${lang}/pull-calc/`;
  const altKo = '/kr/pull-calc/';
  const altEn = '/en/pull-calc/';
  const altJp = '/jp/pull-calc/';
...
function buildExpectedFiles(seoMeta) {
  ...
  for (const lang of LANGS) {
    ...
    const content = normalizeNewline(
      renderPullCalcPage({ lang, title, description })
    );
...
function runCheck(expectedFiles) {
  ...
  if (missing.length || changed.length || extra.length) {
    console.error('Pull-calc SEO pages are out of date.');
    if (missing.length) {
  ...
  console.log(`Pull-calc SEO pages are up to date (${expectedFiles.size} files).`);
}
...
function runGenerate(expectedFiles) {
  ...
  console.log(`Generated ${expectedFiles.size} pull-calc SEO files in ${toPosix(path.relative(ROOT, OUTPUT_DIR))}`);
}
