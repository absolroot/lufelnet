...
후카는 의식0일 경우 기본적으로 2라운드/6라운드에 교체가 강제된다. 의식 6이면 6라운드 기준 추가 턴이 없을 경우 교체가 4번까지 가능하다.
...
2. 후보 캐릭터를 대상으로 한 능력도 작동하지 않음.
   ...
   - 예: 카타야마가 후방일 때는 다운 상태인 적 공격 시 공격력 증가 패시브도 발동하지 않음.
...
At A0, Fuuka is basically forced to swap on rounds 2 and 6. At A6, if there are no extra turns by round 6, she can swap up to 4 times.
...
2. Effects that require the [Backup Character] active will also not work.
    ...
    - Example: When Blitz is inactive, her passive that increases Attack when allies attack downed foes will not activate.
...
A0の場合、基本的に2ラウンド目と6ラウンド目に交代が強制される。A6なら6ラウンド目までに追加ターンがない場合、最大4回まで交代できる。
...
2. 候補キャラクターを対象とする能力も機能しない。
   ...
   - 例: 片山 久未が控えのとき、味方がダウン中の敵を攻撃した時の攻撃力上昇パッシブも発動しない。
3. ただし、候補キャラクターの**デバフ状態を参照する能力**は正常に機能する。
   - 例: 桐条 美鶴の『寒霜結』状態の敵が受けるクリティカル効果上昇は維持される。`,
review_cn: `
山岸风花是一名拥有独特换人能力的解明怪盗，能够通过『愿望聆听』临时与队友进行替换。

和以往的NAVI或ANGE不同，她本身没有防御力降低机制，因此队伍搭配时需要额外留意。

在意识0时，她基本会在第2轮和第6轮被强制换人。若达到意识6，在6轮内没有额外回合的前提下，最多可以完成4次替换。

<br>

**换人机制简述**
- 风花会在战斗开始前指定1名『后备角色』，并通过对应能力完成替换。
- 发动替换后，当前角色会暂时退场，由『后备角色』登场行动1次；随后后备角色退场，原角色回场并继续完成剩余行动。

<br>

**增益交互**
1. 当前角色被替换成『后备角色』时，**除了当前角色自己施加给自己的增益外，其余增益都会复制给后备角色。**
   - 后备角色自己给自己的增益，以及当前角色给自己施加的自我增益，都不会被复制。
2. 相反，当后备角色退场、原角色重新回场时，**不会复制任何增益。**
3. 后备角色行动结束后，被复制过去的增益持续回合数会减少1回合。
  - 例：WONDER身上有麻由美增益（2回合）+ 反叛（3回合），此时风花把WONDER换成绫香
    - 绫香只会复制到麻由美增益（反叛属于WONDER给自己的增益，所以不会复制）
    - 绫香使用麻由美HIGHLIGHT（2回合）
    - 风花再把WONDER换回场上 → 麻由美HIGHLIGHT不会复制回来，WONDER仍保留原本的反叛+麻由美增益

<br>

**后备角色交互**
1. 大多数被动与特性在后排时都不会生效（可视作类似退场或无法行动）。
2. 需要以后备角色在场为前提的能力也不会发动。
   - 例：桐条美鹤在后排时，即使友方攻击了带有『霜结』的敌人，『冷血之刃』也不会发动。
   - 例：片山久未在后排时，友方攻击处于倒地状态的敌人时触发的攻击力提升被动也不会发动。
3. 但若能力本身只是**参照后备角色留下的减益状态**，则依然能够正常运作。
   - 例：桐条美鹤留下的『寒霜结』相关受暴击效果提升仍会继续生效。`,
        dump:``,
