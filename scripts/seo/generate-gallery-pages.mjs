...
/**
 * generate-gallery-pages.mjs
 *
 * Generates static SEO pages for the gallery app in each supported language.
 *
 * Usage:
 *   node scripts/seo/generate-gallery-pages.mjs
 *   node scripts/seo/generate-gallery-pages.mjs --check
 */
...
const OUTPUT_DIR = path.join(ROOT, 'pages', 'gallery');
const LANGS = ['kr', 'en', 'jp'];
const IMAGE_PATH = '/assets/img/home/SEO.png';

const seoMetaPath = path.join(ROOT, 'i18n', 'pages', 'gallery', 'seo-meta.json');
...
function ensureSeoMetaShape(meta) {
  for (const lang of LANGS) {
    ...
    if (typeof langMeta.title !== 'string' || !langMeta.title) {
      throw new Error(`i18n/pages/gallery/seo-meta.json missing valid title for ${lang}.`);
    }
    if (typeof langMeta.description !== 'string' || !langMeta.description) {
      throw new Error(`i18n/pages/gallery/seo-meta.json missing valid description for ${lang}.`);
    }
  }
}

function renderGalleryPage({ lang, title, description }) {
  const permalink = `/${lang}/gallery/`;
  const altKo = '/kr/gallery/';
  const altEn = '/en/gallery/';
  const altJp = '/jp/gallery/';
...
function buildExpectedFiles(seoMeta) {
  ...
  for (const lang of LANGS) {
    ...
    const content = normalizeNewline(
      renderGalleryPage({ lang, title, description })
    );
...
function runCheck(expectedFiles) {
  ...
  if (missing.length || changed.length || extra.length) {
    console.error('Gallery SEO pages are out of date.');
    if (missing.length) {
  ...
  console.log(`Gallery SEO pages are up to date (${expectedFiles.size} files).`);
}
...
function runGenerate(expectedFiles) {
  ...
  console.log(`Generated ${expectedFiles.size} gallery SEO files in ${toPosix(path.relative(ROOT, OUTPUT_DIR))}`);
}
