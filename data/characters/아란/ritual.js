window.ritualData = window.ritualData || {};
window.enCharacterRitualData = window.enCharacterRitualData || {};
window.jpCharacterRitualData = window.jpCharacterRitualData || {};
window.ritualData["아란"] = {
  "name": "히라노 아란",
  "r0": "수잔나의 거울",
  "r0_detail": "히라노 아란은 적에게 특수 상태인 『죄』를 부여할 수 있으며, 전장에 동시에 최대 2개의 『죄』가 존재할 수 있다.\n『죄』: 『죄』를 보유한 적이 받는 대미지의 10%를 『죄』 상태인 다른 적에게 복제한다. 대상이 동일한 적일 경우 자신이 받는 대미지의 5%를 자신에게 복제한다. 『죄』 보유자가 처치되면 다른 생존 유닛에게 전이된다. 『죄』의 전도 비율은 전도 효과 증폭의 영향을 받는다.",
  "r1": "주디스의 단검",
  "r1_detail": "『죄업』 1중첩당 적이 받는 대미지 5% 증가 효과를 추가로 부여한다. 『복수의 명경』의 기본 반사 대미지 비율이 400%로 증가한다.",
  "r2": "야엘의 못 자국",
  "r2_detail": "『금전의 식흔』과 『복수의 명경』이 부여하는 『죄업』 중첩수가 1단계 추가 증가한다. 『죄업』의 최대 중첩수가 2단계 추가 증가한다.",
  "r3": "화가 여신의 구도",
  "r3_detail": "『금전의 식흔』, 『전투 기술』의 스킬 레벨이 2레벨 증가하며, 최대 15레벨까지 증가한다.",
  "r4": "성 카트린의 수레바퀴",
  "r4_detail": "HIGHLIGHT 효과 증가: HIGHLIGHT 에너지 15포인트를 반환한다. 『죄』 상태인 적이 HIGHLIGHT 공격을 받을 시 해당 대미지에 공격력 25%의 보너스를 3턴 동안 추가하며, 이 효과를 『죄』 상태인 다른 적에게 복제한다.",
  "r5": "막달라의 고행",
  "r5_detail": "『단죄의 눈』, 『복수의 명경』의 스킬 레벨이 2레벨 증가하며, 최대 15레벨까지 증가한다.",
  "r6": "에스더의 반역 왕좌",
  "r6_detail": "『죄』 상태인 서로 다른 적이 동시에 대미지 대상으로 선택될 경우 전도 효과가 추가로 150% 증폭된다. 『죄』 상태인 적은 자신이 받는 다운 수치 대미지, 정신 이상 및 원소 이상 상태를 『죄』 상태인 다른 적에게 전도할 수 있다."
};

window.enCharacterRitualData["아란"] = {
  "name": "Aran Hirano",
  "r0": "Susanna's Mirror",
  "r0_detail": "Aran can inflict [Sin] on foes, with a maximum of 2 [Sin] stacks active on the field.\n[Sin]: 10% of damage taken by a foe with [Sin] is copied to other foes with [Sin]. If the target is the same unit, 5% of damage taken is copied to itself. When a foe with [Sin] is defeated, [Sin] transfers to another surviving unit. The conduction ratio of [Sin] is affected by conduction amplification effects.",
  "r1": "Judith's Dagger",
  "r1_detail": "Each [Guilt] stack additionally increases damage taken by 5%. Increase the base reflection ratio of [Mirror of Vengeance] to 400%.",
  "r2": "Jael's Nail",
  "r2_detail": "Increase [Guilt] stacks inflicted by [Forbidden Scars] and [Mirror of Vengeance] by 1. Increase the maximum limit of [Guilt] stacks by 2.",
  "r3": "Composition of the Muse",
  "r3_detail": "Increase the skill levels of [Forbidden Scars] and [Thief Tactics] by 2.",
  "r4": "Saint Catherine's Wheel",
  "r4_detail": "Highlight Enhanced: Restore 15 Highlight Energy. When a foe with [Sin] is hit by a HIGHLIGHT, increase the damage by 25% of Attack for 3 turns, and copy this effect to other foes with [Sin].",
  "r5": "Magdalene's Penance",
  "r5_detail": "Increase the skill levels of [Eyes of Judgment] and [Mirror of Vengeance] by 2.",
  "r6": "Esther's Rebellious Throne",
  "r6_detail": "When different foes with [Sin] are selected as damage targets simultaneously, increase conduction effect by 150%. Foes with [Sin] can also conduct Down Points damage, mental ailments, and elemental ailments to other foes with [Sin]."
};

window.jpCharacterRitualData["아란"] = {
  "name": "平野亜蘭",
  "r0": "スザンナの鏡",
  "r0_detail": "平野亜蘭は敵に対して特殊状態の『罪』を付与でき、場に最大2つの『罪』が同時に存在できる。\n『罪』：『罪』を持つ敵が受けたダメージの10%を、『罪』状態の他の敵にコピーする。対象が同一の敵である場合、自身が受けたダメージの5%を自身にコピーする。『罪』を持つ敵が倒れた場合、『罪』は他の生存ユニットに移譲される。『罪』の伝導率は、伝導効果増幅の影響を受ける。",
  "r1": "ユディトの短剣",
  "r1_detail": "『罪業』1スタックにつき、被ダメージが5%上昇する効果をさらに付与する。『復讐の冥鏡』の基本反射ダメージ倍率が400%に上昇する。",
  "r2": "ヤエルの釘跡",
  "r2_detail": "『禁典の蝕痕』と『復讐の冥鏡』が付与する『罪業』のスタック数が1段階追加で増加する。『罪業』の上限が2段階追加で増加する。",
  "r3": "絵画の女神の構図",
  "r3_detail": "『禁典の蝕痕』と『怪盗戦技』のスキルレベルが2上昇し、最大15レベルまで上昇する。",
  "r4": "聖カタリナの車輪",
  "r4_detail": "ハイライト能力追加：HIGHLIGHTエネルギーを15ポイント返還する。『罪』状態の敵がHIGHLIGHT攻撃を受ける際、そのダメージに攻撃力25%分のボーナスを3ターンの間追加し、その効果を『罪』状態の他の敵にコピーする。",
  "r5": "マグダラの方行",
  "r5_detail": "『断罪の瞳』と『復讐の冥鏡』のスキルレベルが2上昇し、最大15レベルまで上昇する。",
  "r6": "エステルの反逆の王座",
  "r6_detail": "『罪』状態の異なる敵が同時にダメージ対象として選択された場合、伝導効果がさらに150%増幅される。『罪』状態の敵は、自身が受けたダウン値ダメージ、精神異常、および元素異常を『罪』状態の他の敵に伝導できるようになる。"
};
window.cnCharacterRitualData = window.cnCharacterRitualData || {};
window.cnCharacterRitualData["아란"] = {
  "name": "",
  "r0": "",
  "r0_detail": "",
  "r1": "",
  "r1_detail": "",
  "r2": "",
  "r2_detail": "",
  "r3": "",
  "r3_detail": "",
  "r4": "",
  "r4_detail": "",
  "r5": "",
  "r5_detail": "",
  "r6": "",
  "r6_detail": ""
};
