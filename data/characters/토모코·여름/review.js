(function () {
    window.characterReview = window.characterReview || {};
    window.characterReview["토모코·여름"] = { name_en: "Seaside Tomoko", name_jp: "朋子・ピーチサマー", codename: "MOKO·Seaside",
        review: `
아군 딜러에게는 **효과명중**과 공격력을 올려주고, 적은 받는 대미지를 증가시키는 힐러다.

공격과 힐, 버프가 모두 가능하지만 최상의 값은 아닌 밸런스형으로 스킬 셋이 구성돼있다.

스킬 사이클은 S2-S3의 반복으로 간단한 편이며 효과 명중을 올려주는 만큼 관련 스탯에 영향을 받는 딜러들과 잘 어울린다. 

<br>

**[조화]-[승리]** 계시 카드 세트의 경우, [화려한 불꽃]이 다단 히트를 수행하므로 **세트 효과 발동이 비교적 쉬운 편**이라 **[창조]-[화해]** 세트의 등장 전까지는 우선 선택된다.

필수적으로 세팅해야할 스탯 값이 없기 때문에 비교적 **카드 세팅이 편한 편**이다.
`,
review_en: `
A healer who increases **Ailment Accuracy** and Attack for allied dealers, while also increasing damage taken by enemies.

Although she can attack, heal, and buff, her skill set is built as a balanced type rather than focusing on one strength.

Her skill cycle is simple, mainly repeating S2–S3, and she pairs well with dealers who benefit from stats affected by increased **Ailment Accuracy**.

<br>

For the **[Harmony - Victory]** Revelation Card Set, since **[Summer Reminiscence]** performs multi-hit attacks, the **set effect is easier to trigger**, making it the preferred choice until the **[Creation - Reconciliation]** set is released.

Because there are no mandatory stat requirements, **card setup is relatively easy**.
`,
 
review_jp: `
味方のアタッカーに**状態異常命中**と攻撃力を上昇させ、敵には被ダメージ増加を付与するヒーラー。

攻撃・回復・バフのすべてが可能だが、突出した性能というよりバランス型のスキル構成になっている。

スキルサイクルはS2→S3の繰り返しでシンプルであり、**状態異常命中**に影響を受けるアタッカーと非常に相性が良い。

<br>

**[調和 - 勝利]**の啓示カードセットでは、**[ひと夏の思い出]**が多段ヒットを行うため、**セット効果の発動が比較的容易**であり、**[創造 - 和解]**セットが登場するまでは優先的に選ばれる。

必須ステータスの指定がないため、**カード構成が比較的簡単**なのも特徴。
`,
        review_cn: `是能够为我方输出角色提高**效果命中**与攻击力，同时让敌人受到伤害提升的治疗角色。

她虽然兼具攻击、治疗和增益能力，但整体是偏均衡型的技能组，各方面都能做却不是某一项极端突出。

技能循环基本上以S2和S3轮转为主，操作比较简单；同时因为她能提供效果命中，所以和那些会从该属性中获益的输出角色相性很好。

<br>

若使用**[和谐]-[告捷]**启示套装，由于**[绚烂烟火]**本身是多段攻击，所以**触发套装效果相对容易**，在**[创造]-[和解]**套装出现之前通常会优先选择这一套。

因为没有必须卡死的关键属性线，所以她的**卡组配置相对轻松**。`,
        pros: ["효과 명중을 올려주는 몇 없는 버프 캐릭터이다.", "불꽃놀이를 통해 꾸준한 회복을 가능하게 한다."],
        pros_en: ["It is a rare buff character that increases Ailment Accuracy.", "It allows for regular healing through Summer Fireworks."],
        pros_jp: ["状態異常命中を上昇させる珍しいバファーです。", "夏の花火を通じて定期的な回復を可能にします。"],
        pros_cn: ["是少数能够提高效果命中的增益角色。", "能够通过『绚烂烟火』实现稳定持续的恢复。"],
        cons: ["밸런스형인 만큼 특출난 부분이 부족하다."],
        cons_en: ["Since it is a balanced type, it lacks a standout feature."],
        cons_jp: ["バランス型なので、特徴的な部分が不足しています。"],
        cons_cn: ["正因为是均衡型角色，所以缺少特别突出的强项。"],
    };
})();

