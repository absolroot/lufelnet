 ...
 *
 * Generates static SEO pages for the homepage in each supported language.
 *
 * Source priority:
 *   1. SEO meta: i18n/pages/home/seo-meta.json
 *   2. Image: /assets/img/home/SEO.png (shared across all languages)
 *
 * Usage:
 *   node scripts/seo/generate-home-pages.mjs
 *   node scripts/seo/generate-home-pages.mjs --check
 */
...
const OUTPUT_DIR = path.join(ROOT, 'pages', 'home');
const LANGS = ['kr', 'en', 'jp'];
const IMAGE_PATH = '/assets/img/home/SEO.png';

const seoMetaPath = path.join(ROOT, 'i18n', 'pages', 'home', 'seo-meta.json');
...
function ensureSeoMetaShape(meta) {
  for (const lang of LANGS) {
    ...
    if (typeof langMeta.title !== 'string' || !langMeta.title) {
      throw new Error(`i18n/pages/home/seo-meta.json missing valid title for ${lang}.`);
    }
    if (typeof langMeta.description !== 'string' || !langMeta.description) {
      throw new Error(`i18n/pages/home/seo-meta.json missing valid description for ${lang}.`);
    }
  }
}

function renderHomePage({ lang, title, description }) {
  const permalink = `/${lang}/`;
  const altKo = '/kr/';
  const altEn = '/en/';
  const altJp = '/jp/';
  ...
  return [
    '---',
    'layout: default',
    'custom_css: [tactic-posts]',
    'custom_js: [tactic-preview, /tactic/share]',
...
function buildExpectedFiles(seoMeta) {
  ...
  for (const lang of LANGS) {
    ...
    const content = normalizeNewline(
      renderHomePage({ lang, title, description })
    );
...
function runCheck(expectedFiles) {
  ...
  if (missing.length || changed.length || extra.length) {
    console.error('Home SEO pages are out of date.');
    if (missing.length) {
  ...
  console.log(`Home SEO pages are up to date (${expectedFiles.size} files).`);
}
...
function runGenerate(expectedFiles) {
  ...
  console.log(`Generated ${expectedFiles.size} home SEO files in ${toPosix(path.relative(ROOT, OUTPUT_DIR))}`);
}
