...
**아군의 턴 시작 시에 SP를 15 회복**하며 **자신의 턴 시작에 SP를 10 회복**합니다. 두 수치는 SP회복 수치가 적용되며 이를 활용해 최대한 많은 추가 효과를 발동시키는 것이 중요합니다. 

<br>

SP회복에 큰 영향을 받는 캐릭터지만 **기본 188.5%** SP회복과 계시 카드 진의 **SP 회복 주 옵션 90.4%**, 마이팰리스 5%만 추가하더라도 **283.9%의 SP 회복**을 쉽게 확보할 수 있습니다.

의식이 6에 도달할 경우 추가 SP를 250을 주므로, 별도의 SP회복이 필요 없어집니다. (SP 280%까지는 패시브에 의해 변환되므로 공격력 손해가 없습니다.)
...
첫 3가지의 약품은 무제한으로 사용 할 수 있고, 나머지 **아이템은 구매해야 하지만 모두 독립적인 대미지 버킷으로 계산**됩니다. 따라서 **약품 효과 증가**의 가치가 높습니다.

스킬3을 사용한 후 자신의 턴에 버프를 소모해 행동을 소모하지 않고 아이템을 사용할 수 있고, 해당 버프를 통해 사용된 아이템은 지속시간이 1턴 늘어납니다. 
...
이치고는 정말 강력하지만 딜링 메커니즘은 역대 캐릭터 중 가장 복잡합니다. 
...
- 적이 턴을 끝낼 때 받는 지속대미지 결산의 경우 이치고의 관통 버프가 적용되지 않아, 방어력 감소 효과가 다른 캐릭터들보다 유의미합니다.

---

아래는 이치고의 버프 상태가 적용되는 버프 테이블입니다.
...
**지속 대미지 증가**라는 버프가 기존 대미지 증가와는 다르게 별도의 버킷으로 계산됩니다.

참고로 이치고가 나타나기 전에 출시됐던 '년수'의 지속 대미지 증가는 일반적인 대미지 증가 버킷으로 계산됩니다.
...
그러나 아야카의 **스킬3은 쿨다운에 관계없이 강화된 HL을 강제로 발동 가능한 스킬**로 **6턴 동안 최대 5번의 HL**을 사용 가능하게 만듭니다.

<br>

스킬2를 통해 동료의 공격력을 올릴 수 있고, **의식1**로 업그레이드 할 경우 **크리티컬 확률을 15%** 올려주는 버프를 제공합니다.

아야카의 HL은 아군에게 버프를 제공하면서 일정 시간 HL 충전량을 올려 보다 압축된 HL 사이클을 만들어내는 등 전체적인 대미지를 크게 상승시킵니다.

**HL에 대미지를 의존하는 파티라면 무조건적으로 채용**되며, 그렇지 않더라도 **어디에나 함께 사용 가능한 범용 서포터**입니다.
...
해당 이유 때문에 **1돌5재**라는 괴상한 캐릭터 형태가 효율이 뛰어난 캐릭터 강화 요소로 인정받고 있습니다.
`,
review_en: `
...
しかし、アヤカの**スキル3はクールダウンに関係なく強化HLを強制的に発動できるスキル**であり、**6ターンの間に最大5回のHL**を使用可能にします。

<br>

スキル2によって味方の攻撃力を上昇させ、さらに**意識1**にアップグレードすると**CRT発生率を15%**上昇させるバフを付与します。

アヤカのHLは味方にバフを与えるだけでなく、一定時間HLゲージの充填量を増加させ、より圧縮されたHLサイクルを作り出し、全体的なダメージを大幅に向上させます。
...
"유우미": { name_en: "Yumi Shiina", name_jp: "椎名 悠美", codename: "PHOEBE",
        review: `
유우미(PHOEBE)는 강력한 속성 대미지 보너스를 부여하는 해명 괴도로 동일 속성의 아군이 많을 수록 가치가 높아집니다.

아군이 속성 공격을 할 때마다 중첩이 1개씩 쌓이게 되고 **중첩이 3개가 되면 칵테일을 제조**합니다. **동일 속성의 중첩을 모을수록 높은 품질의 칵테일을 생성**합니다.

만약 품질이 낮은 칵테일을 생성하더라도 1개는 스킬2를 통해 최고 등급의 칵테일로 변환할 수 있고, **스킬3을 통해 특정 속성의 대미지를 크게 증가**시킬 수 있습니다.

<br>

요구 공격력이 높은 편이라 계시 카드 세팅을 신경써줘야하고, 전용 무기가 공격력을 올려줄 뿐 아니라 행동 턴 수를 변화시킬 수 있는 요소가 있어 전용 무기를 신경 쓰는 편이 좋습니다.

최초의 5성 해명 괴도로서 기존 15%씩 스탯을 아군에게 나눠주던 4성과 달리, **아군에게 스탯의 20%를 나눠줌으로서 해명 괴도의 스탯을 보다 신경써야 합니다.**

칵테일에 의존하는 캐릭터로 의식6에 칵테일의 상한이 3잔에서 4잔으로 늘어나는 영향도가 높습니다.
`,
review_en: `
