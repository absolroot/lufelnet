window.I18N_PAGE_PULL_TRACKER_JP = {
  ...
  guide: {
    ...
    pageTitle: 'URL ガイド',
    introWindows: '現在は Windows / iOS / Linux / Steam Deck 向けの方法を提供しています。',
    s1: 'FAQ を確認してください。',
    ...
    linux1: 'httptap をインストールします。Steam Deck の場合は先にデスクトップモードへ切り替えてください。',
    linux2: 'ターミナルを開いて、次のインストールコマンドを実行します。',
    ...
    linux3: 'Steam が起動中なら終了し、次のコマンドを実行します。',
    linuxRunCmd: './httptap -- steam 2>/dev/null | grep "X"',
    linux4: 'grep の "X" を自分のサーバードメインに置き換えます（引用符はそのまま）。\n(KR) nsywl-krcard.wmupd.com\n(CN) nsywl-gfcard.wmupd.com\n(TW) wwwidcweb.p5x.com.tw\n(JP) web.p5xjpupd.com\n(EN) euweb.p5xjpupd.com\n(SEA) web.p5x-sea.com',
    linux5: 'Steam 起動時に SSL certificate の警告が表示されますが、無視して構いません。',
    linux6: 'P5X を起動し、ガチャ履歴画面を開きます。',
    linux7: 'ターミナルに戻るとガチャ履歴URLが表示されます（通常2回表示）。URLを1つ丸ごとコピーします。',
    linux8: '下のリンクからコピーした URL を入力してください。',
    linuxExplain: 'コマンドの説明:\n./httptap -- steam: Steam を httptap 経由で起動し、Steam/P5X の HTTP リクエストを表示します。\n2>/dev/null: Steam のデバッグ出力を非表示にします。\n| grep "X": サーバードメイン(X)を含む行だけを抽出します。',
    linuxCredit: 'この手順は Dinjoralo さんの提供です。',
    videoLabel: '動画ヘルプ',
