(function () {
    window.characterReview = window.characterReview || {};
    window.characterReview["준페이"] = { name_en: "Junpei Iori", name_jp: "伊織順平", codename: "JUNPEI",
        review: `P3의 오리지널 캐릭터이자 화염 속성 딜러를 보조하는 탱커다.
        
특이점으로 범용성을 포기하고 **화염 속성 한정으로 방위 속성이라는 기존 역할의 한계를 뛰어넘는 강력한 버프를 제공**한다.

스킬2는 범용성을 위해 존재하나 스킬1의 하위호환으로 일반적으로는 스킬3과 스킬1과 테우르기아의 반복 사용인 2턴 사이클이 정석이 된다.
`,
        review_en: `An original P3 character, he’s a tank who supports Fire-attribute DPS units.

His standout trait is that he sacrifices general-purpose utility and, **specifically for Fire teams, provides extremely powerful buffs that push past the usual limits of a Defense-role character**.

Skill 2 exists mainly for flexibility, but it’s generally a weaker version of Skill 1. In most cases, the standard play becomes a 2-turn cycle that repeatedly uses Skill 3, Skill 1, and Theurgy.

`,
        review_jp: `P3のオリジナルキャラクターで、火炎属性アタッカーを支えるタンク。

特徴として汎用性を捨て、**火炎属性に限定して、防衛という既存ロールの限界を超える強力なバフを提供**できる。

スキル2は汎用性のために存在するが、基本的にはスキル1の下位互換。一般的にはスキル3・スキル1・テウルギアを繰り返す2ターンサイクル運用が定石となる。

`,
        review_cn: `P3的原创角色，是一名专门辅助火焰属性输出的坦克。

他最大的特点在于舍弃了泛用性，**只针对火焰属性队伍提供了远超一般防卫角色上限的强力增益**。

技能2虽然是为了补泛用性而存在，但整体上还是技能1的下位替代，因此实战里通常会以技能3、技能1与神通法反复循环的2回合轴作为标准玩法。
`,
        pros: [
          "화염 속성 반항/지배 딜러에게 매우 높은 수치의 버프를 제공한다.",
          "아군의 생존력을 높여준다.",
        ],
        pros_en: [
          "Provides very high bonus buffs to Fire attribute DPS.",
          "Increases the survival of allies.",
        ],
        pros_jp: [
          "火炎属性ダメージデュースに対して非常に高いボーナスを提供します。",
          "味方の生存率を高めます。",
        ],
        pros_cn: [
          "能为火焰属性的反抗/支配主C提供很高数值的增益。",
          "可以显著提高全队生存能力。"
        ],
        cons: [
          "화염 속성 딜러가 파티에 없는 경우 효과가 급감한다."
        ],
        cons_en: [
          "The effect drops significantly when there is no Fire attribute DPS in the party."
        ],
        cons_jp: [
          "パーティーに火炎属性ダメージデュースがいない場合、効果が大幅に低下します。"
        ],
        cons_cn: [
          "如果队伍里没有火焰属性输出，价值会明显下滑。"
        ],
    };
})();


