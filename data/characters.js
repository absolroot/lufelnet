      
// 캐릭터 목록 데이터
const characterList = {
    mainParty: [
        "렌", "루우나", "루페르", "레오", "류지",
        "리코·매화", "마사키", "마코토", "미나미", "모르가나",
        "모토하", "모토하·여름", "몽타뉴", "몽타뉴·백조", "미오", "슌",
        "세이지", "안", "아야카", "아케치", "야오링", "야오링·사자무",
        "원더", "유스케", "유이 YUI", "유키미", "카스미",
        "키라", "키요시", "토모코", "토모코·여름", "토시야",
        "하루", "하루나", "치즈코"
    ],
    supportParty: [
        "리코", "미유", "유우미", "카요", "후타바"
    ]
};

/* 
    color : 택틱 메이커 등에서 나타날 대표 색상
    main_revelation : 추천 주 계시
    sub_revelation : 추천 일월성진 계시
    element : 속성
    rarity : 4성/5성 
    position : 지배/굴복/구원/반항/해명/우월/구원/반항/해명
    release_order : 캐릭터 출시 순서
    codename : 코드네임
    persona : 페르소나 이름
    name : 캐릭터 이름
    sub_revel2 : 월 주옵션
    sub_revel3 : 성 주옵션
    sub_revel4 : 진 주옵션 
    sub_option1 : 부옵션 추천 1순위
    sub_option2 : 부옵션 추천 2순위
    sub_option3 : 부옵션 추천 3순위
    minimum_stats : 스킬 옵션을 위한 권장 스탯
    battle_plus_stats : 권장 스탯에 영향 주는 요소
    skill1_lv : 스킬1 추천 레벨
    skill2_lv : 스킬2 추천 레벨
    skill3_lv : 스킬3 추천 레벨
    skill4_lv : HL/전투기술 추천 레벨
    mind_stats1 : 심상 스텟 1
    mind_stats2 : 심상 스텟 2
    mind_skill1 : 심상 스킬 1
    mind_skill2 : 심상 스킬 2
*/

const characterData = {
    "원더": {
       "role": "",
       "tag":"",
        "color": "#000000",
    },
    "아케치": {
       "role": "만능 광역 딜러 (서브)",
       "tag":"대미지 축적, 대미지보너스, 방어력 감소, 공격력, 크리티컬효과, 축복",
        "color": "#c7312c",
        "main_revelation": ["자유"],
        "sub_revelation": ["실망"],
        "element": "만능",
        "element_weakness": "",
        "element_resistance": "",
        "rarity": 5,
        "position": "지배",
        "release_order": 21,
        "persona": "로빈후드",
        "name": "아케치 고로",
        "codename": "CROW",
        "sub_revel2": ["공격력", "대미지보너스"],
        "sub_revel3": ["크리티컬효과", "크리티컬확률"],
        "sub_revel4": ["공격력"],
        "sub_option1": ["크리티컬확률", "크리티컬효과"],
        "sub_option2": ["공격력", "대미지보너스"],
        "sub_option3": ["-"],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": false,
            "LV12+5": false,
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "nan",
        "skill1_lv": "MAX",
        "skill2_lv": "MAX",
        "skill3_lv": "MAX!",
        "skill4_lv": "MAX",
        "mind_stats1": "미확인",
        "mind_stats2": "미확인",
        "mind_skill1": "미확인",
        "mind_skill2": "미확인"
    },

    "미오": {
        "role": "광역 디버퍼",
        "tag": "방어력 감소, 받는 대미지 증가,빙결 속성 TECHNICAL, 실드, 스킬마스터, 동결",
        "color": "#4287f5",
        "main_revelation": ["신념"],
        "sub_revelation": ["헛수고"],
        "element": "빙결",
        "element_weakness": "축복",
        "element_resistance": "빙결",
        "rarity": 5,
        "position": "굴복",
        "release_order": 20,
        "persona": "민테",
        "name": "나츠카와 미오",
        "codename": "MATOI",
        "sub_revel2": [""],
        "sub_revel3": ["효과명중"],
        "sub_revel4": [""],
        "sub_option1": ["효과명중"],
        "sub_option2": [""],
        "sub_option3": [""],
        "minimum_stats": {
            "LV10": "효과명중 171%",
            "LV10+5": "효과명중 189%",
            "LV12": false,
            "LV12+5": false,
            "LV13": "효과명중 182%",
            "LV13+5": "효과명중 200%"
        },
        "battle_plus_stats": "· 마이팰리스 2.3% / 헛수고 : 2턴 동안 30% 효과명중 증가",
        "skill1_lv": "MAX!",
        "skill2_lv": "8",
        "skill3_lv": "MAX!",
        "skill4_lv": "MAX",
        "mind_stats1": "대미지보너스 16%",
        "mind_stats2": "공격력 20%",
        "mind_skill1": "1/2!",
        "mind_skill2": "3/HL!"
    },
    "레오": {
       "role": "단일 버퍼",
       "tag":"공격력, 크리티컬효과, 실드",
        "color": "#01dfd7",
        "main_revelation": ["신뢰", "신뢰", "창조"],
        "sub_revelation": ["풍요", "힘", "화해"],
        "element": "핵열",
        "element_weakness": "화염",
        "element_resistance": "핵열",
        "rarity": 4,
        "position": "우월",
        "release_order": 0,
        "persona": "에리테이아",
        "name": "카미야마 레오",
        "codename": "LEON",
        "sub_revel2": ["공격력","생명"],
        "sub_revel3": ["공격력","생명"],
        "sub_revel4": ["공격력","생명"],
        "sub_option1": ["생명","방어"],
        "sub_option2": ["공격력"],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": "공격력 4064",
            "LV12+5": "공격력 4465",
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "· 해명의힘 / 직책 8% / 마이팰리스 50",
        "skill1_lv": "8",
        "skill2_lv": "8",
        "skill3_lv": "MAX!",
        "skill4_lv": "MAX!",
        "mind_stats1": "생명 20%",
        "mind_stats2": "공격력 20%×",
        "mind_skill1": "1/HL!",
        "mind_skill2": "2/3!"
    },
    "렌": {
       "role": "주원 광역 딜러",
       "tag":"추가 턴, 주원",
        "color": "#212529",
        "main_revelation": ["여정", "여정"],
        "sub_revelation": ["방해", "주권"],
        "element": "주원",
        "element_weakness": "축복",
        "element_resistance": "주원",
        "rarity": 5,
        "position": "지배",
        "release_order": 2,
        "persona": "아르센",
        "name": "아마미야 렌",
        "codename": "JOKER",
        "sub_revel2": ["공격력", "대미지보너스"],
        "sub_revel3": ["크리티컬효과", "크리티컬확률"],
        "sub_revel4": ["공격력"],
        "sub_option1": ["크리티컬확률", "크리티컬효과"],
        "sub_option2": ["공격력", "대미지보너스"],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": false,
            "LV12+5": false,
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "nan",
        "skill1_lv": "MAX",
        "skill2_lv": "8",
        "skill3_lv": "MAX!",
        "skill4_lv": "MAX!",
        "mind_stats1": "대미지보너스 16%",
        "mind_stats2": "크리티컬확률 12%!",
        "mind_skill1": "1/HL!",
        "mind_skill2": "2/3!"
    },
    "루우나": {
       "role": "단일/화염 디버퍼",
       "tag":"추가효과 단일,화염 광역,받는 대미지 증가, 방어력 감소, 화상",
        "color": "#ff69b4",
        "main_revelation": ["자유", "여정"],
        "sub_revelation": ["좌절", "주권"],
        "element": "화염",
        "element_weakness": "질풍",
        "element_resistance": "화염",
        "rarity": 5,
        "position": "굴복",
        "release_order": 16,
        "persona": "아우라",
        "name": "도겐자카 루우나",
        "codename": "HOWLER",
        "sub_revel2": ["-"],
        "sub_revel3": ["효과명중"],
        "sub_revel4": ["-"],
        "sub_option1": ["효과명중"],
        "sub_option2": ["공격력", "대미지보너스", "크리티컬확률"],
        "sub_option3": ["생명", "방어력"],
        "minimum_stats": {
            "LV10": "효과명중 170%",
            "LV10+5": "효과명중 190%",
            "LV12": false,
            "LV12+5": false,
            "LV13": "효과명중 182%",
            "LV13+5": "효과명중 199%"
        },
        "battle_plus_stats": "· 전용무기 : 전투 중 효과명중 증가 / 마이팰리스 2.3%",
        "skill1_lv": "MAX!",
        "skill2_lv": "MAX!",
        "skill3_lv": "MAX!",
        "skill4_lv": "MAX!",
        "mind_stats1": "대미지보너스 16%",
        "mind_stats2": "공격력 20%",
        "mind_skill1": "3/HL!",
        "mind_skill2": "1/2!"
    },
    "루페르": {
       "role": "힐러",
       "tag":"",
        "color": "#4b0082",
        "main_revelation": ["수락"],
        "sub_revelation": ["사랑"],
        "element": "화염",
        "element_weakness": "질풍",
        "element_resistance": "화염",
        "rarity": 4,
        "position": "구원",
        "release_order": 0,
        "persona": "로브로이",
        "name": "루페르",
        "codename": "CATTLE",
        "sub_revel2": ["치료효과"],
        "sub_revel3": ["공격력"],
        "sub_revel4": ["공격력"],
        "sub_option1": ["-"],
        "sub_option2": [],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": false,
            "LV12+5": false,
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "nan",
        "skill1_lv": "8",
        "skill2_lv": "8",
        "skill3_lv": "8",
        "skill4_lv": "8",
        "mind_stats1": "-",
        "mind_stats2": "-",
        "mind_skill1": "-",
        "mind_skill2": "-"
    },
    "류지": {
       "role": "물리 단일 딜러",
       "tag":"차지",
        "color": "#ffff00",
        "main_revelation": ["전념"],
        "sub_revelation": ["용맹"],
        "element": "물리",
        "element_weakness": "질풍",
        "element_resistance": "전격",
        "rarity": 5,
        "position": "반항",
        "release_order": 1,
        "persona": "캡틴 키드",
        "name": "사카모토 류지",
        "codename": "SKULL",
        "sub_revel2": ["공격력", "대미지보너스"],
        "sub_revel3": ["크리티컬효과"],
        "sub_revel4": ["공격력"],
        "sub_option1": ["크리티컬효과"],
        "sub_option2": ["공격력", "대미지보너스"],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": false,
            "LV12+5": false,
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "nan",
        "skill1_lv": "8",
        "skill2_lv": "8",
        "skill3_lv": "MAX!",
        "skill4_lv": "MAX",
        "mind_stats1": "대미지보너스 16%",
        "mind_stats2": "크리티컬효과 24%!",
        "mind_skill1": "1/HL",
        "mind_skill2": "2/3!"
    },
    "리코": {
       "role": "ONE MORE/총공격 서포터",
       "tag":"방어력 감소, 받는 대미지 증가, 대미지보너스, 해명의 힘",
        "color": "#dc143c",
        "main_revelation": ["결심"],
        "sub_revelation": ["직책"],
        "element": "보조",
        "element_weakness": "",
        "element_resistance": "",
        "rarity": 4,
        "position": "해명",
        "release_order": 8,
        "persona": "치요메",
        "name": "타네무라 리코",
        "codename": "WIND",
        "sub_revel2": ["공격력", "대미지보너스"],
        "sub_revel3": ["크리티컬효과", "크리티컬확률"],
        "sub_revel4": ["속도"],
        "sub_option1": ["속도"],
        "sub_option2": ["크리티컬확률", "크리티컬효과"],
        "sub_option3": ["공격력", "대미지보너스"],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": "속도 170",
            "LV12+5": "속도 180",
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "· 마이팰리스 1",
        "skill1_lv": "MAX!",
        "skill2_lv": "MAX",
        "skill3_lv": "MAX!",
        "skill4_lv": "-",
        "mind_stats1": "생명 20%",
        "mind_stats2": "공격력 20%",
        "mind_skill1": "1/2!",
        "mind_skill2": "3!"
    },
    "리코·매화": {
       "role": "단일/광역 버퍼",
       "tag":"크리티컬효과, 크리티컬확률, 공격력, 대미지보너스, 풍습, SP 회복",
        "color": "#c32233",
        "main_revelation": ["창조", "여정"],
        "sub_revelation": ["우려", "주권"],
        "element": "질풍",
        "element_weakness": "염동",
        "element_resistance": "질풍",
        "rarity": 5,
        "position": "우월",
        "release_order": 18,
        "persona": "치요메",
        "name": "타네무라 리코·매화",
        "codename": "WIND",
        "sub_revel2": ["-"],
        "sub_revel3": ["크리티컬효과"],
        "sub_revel4": ["SP회복"],
        "sub_option1": ["SP회복", "크리티컬효과"],
        "sub_option2": ["생명", "방어력"],
        "sub_option3": ["공격력", "크리티컬확률"],
        "minimum_stats": {
            "LV10": "SP회복 450.1% , 크리티컬효과 388%",
            "LV10+5": "SP회복 450.1% , 크리티컬효과 418%",
            "LV12": false,
            "LV12+5": false,
            "LV13": "SP회복 450.1% , 크리티컬효과 418%",
            "LV13+5": "SP회복 480.0% (33 사이클) , 크리티컬효과 448%"
        },
        "battle_plus_stats": "<b>SP회복</b>/ 의식 0: 50% / 마이팰리스: 5% <b>크리티컬효과</b>/ 해명의 힘 / 패시브(활기): 84% / 창조우려: 45%",
        "skill1_lv": "MAX",
        "skill2_lv": "MAX",
        "skill3_lv": "MAX!",
        "skill4_lv": "MAX!",
        "mind_stats1": "공격력 20%",
        "mind_stats2": "크리티컬효과 24%!",
        "mind_skill1": "1/HL!",
        "mind_skill2": "2/3!"
    },
    "마사키": {
       "role": "단일/광역 실드",
       "tag":"실드, 방어력, 대미지보너스, 공격받을 확률, 효과 저항, 동결",
        "color": "#3cf2ff",
        "main_revelation": ["신념", "신뢰"],
        "sub_revelation": ["평화", "풍요"],
        "element": "빙결",
        "element_weakness": "축복",
        "element_resistance": "빙결",
        "rarity": 5,
        "position": "방위",
        "release_order": 13,
        "persona": "아말테이아",
        "name": "아시야 마사키",
        "codename": "CHERISH",
        "sub_revel2": ["방어력"],
        "sub_revel3": ["방어력"],
        "sub_revel4": ["방어력"],
        "sub_option1": ["방어력"],
        "sub_option2": [],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": "방어력 6100",
            "LV10+5": "방어력 6725",
            "LV12": false,
            "LV12+5": false,
            "LV13": "방어력 6475",
            "LV13+5": "방어력 7100"
        },
        "battle_plus_stats": "· 해명의힘 / 직책 8% / 마이팰리스 40 / 패시브 24%",
        "skill1_lv": "8",
        "skill2_lv": "MAX!",
        "skill3_lv": "MAX",
        "skill4_lv": "8",
        "mind_stats1": "효과저항 24%×",
        "mind_stats2": "방어력 30%",
        "mind_skill1": "1/2",
        "mind_skill2": "3/HL"
    },
    "마코토": {
        "role": "핵열 단일 딜러",
        "tag":"원소 이상, TECHNICAL, 추가 턴",
        "color": "#000080",
        "main_revelation": ["깨달음"],
        "sub_revelation": ["진리"],
        "element": "핵열",
        "element_weakness": "염동",
        "element_resistance": "핵열",
        "rarity": 5,
        "position": "반항",
        "release_order": 7,
        "persona": "요한나",
        "name": "니지마 마코토",
        "codename": "QUEEN",
        "sub_revel2": ["공격력", "대미지보너스"],
        "sub_revel3": ["크리티컬효과", "크리티컬확률"],
        "sub_revel4": ["공격력"],
        "sub_option1": ["크리티컬확률", "크리티컬효과"],
        "sub_option2": ["공격력", "대미지보너스"],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": false,
            "LV12+5": false,
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "nan",
        "skill1_lv": "8",
        "skill2_lv": "8",
        "skill3_lv": "MAX!",
        "skill4_lv": "MAX!",
        "mind_stats1": "공격력 20%",
        "mind_stats2": "크리티컬확률 12%!",
        "mind_skill1": "2/HL!",
        "mind_skill2": "1/3!"
    },
    "미나미": {
       "role": "광역 힐러",
       "tag":"축복, 디버프 제거, 최대 생명 증가, 대미지보너스, 지속 회복, 관통, 부활",
        "color": "#f7cd80",
        "main_revelation": ["신뢰", "여정"],
        "sub_revelation": ["풍요", "주권"],
        "element": "축복",
        "element_weakness": "핵열",
        "element_resistance": "축복",
        "rarity": 5,
        "position": "구원",
        "release_order": 3,
        "persona": "탈리아",
        "name": "미야시타 미나미",
        "codename": "MARIAN",
        "sub_revel2": ["생명","치료효과"],
        "sub_revel3": ["생명"],
        "sub_revel4": ["생명"],
        "sub_option1": ["생명"],
        "sub_option2": ["방어"],
        "sub_option3": ["SP회복"],
        "minimum_stats": {
            "LV10": "생명 15000",
            "LV10+5": "생명 15000",
            "LV12": false,
            "LV12+5": false,
            "LV13": "생명 15000",
            "LV13+5": "생명 15000"
        },
        "battle_plus_stats": "· 해명의힘 / 직책 8% / 마이팰리스 220",
        "skill1_lv": "8",
        "skill2_lv": "8",
        "skill3_lv": "8",
        "skill4_lv": "8",
        "mind_stats1": "효과저항 24%×",
        "mind_stats2": "생명 20%×",
        "mind_skill1": "2/HL×",
        "mind_skill2": "1/3×"
    },
    "미유": {
        "role": "실드/방어력 서포터",
        "tag":"실드, 방어력, 대미지보너스, 공격력",
        "color": "#78cccc",
        "main_revelation": ["신념", "진정성"],
        "sub_revelation": ["평화", "직책"],
        "element": "보조",
        "element_weakness": "",
        "element_resistance": "",
        "rarity": 4,
        "position": "해명",
        "release_order": 0,
        "persona": "네메르테스",
        "name": "사하라 미유",
        "codename": "PUPPET",
        "sub_revel2": ["방어력", "대미지보너스"],
        "sub_revel3": ["크리티컬효과", "크리티컬확률"],
        "sub_revel4": ["방어력"],
        "sub_option1": ["크리티컬확률", "크리티컬효과"],
        "sub_option2": ["방어력", "대미지보너스",
            "관통"],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": false,
            "LV12+5": false,
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "nan",
        "skill1_lv": "8",
        "skill2_lv": "8",
        "skill3_lv": "MAX",
        "skill4_lv": "-",
        "mind_stats1": "실드효과 14%×",
        "mind_stats2": "방어력 30%",
        "mind_skill1": "1/2",
        "mind_skill2": "3"
    },
    "모르가나": {
        "role": "광역 힐러",
        "tag":"풍습, 크리티컬, 원소 이상 제거, 추가 효과",
        "color": "#111111",
        "main_revelation": ["전념"],
        "sub_revelation": ["사랑"],
        "element": "질풍",
        "element_weakness": "전격",
        "element_resistance": "질풍",
        "rarity": 5,
        "position": "구원",
        "release_order": 1,
        "persona": "조로",
        "name": "모르가나",
        "codename": "MONA",
        "sub_revel2": ["치료효과"],
        "sub_revel3": ["크리티컬확률"],
        "sub_revel4": ["공격력"],
        "sub_option1": ["크리티컬확률"],
        "sub_option2": [],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": false,
            "LV12+5": false,
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "nan",
        "skill1_lv": "8",
        "skill2_lv": "8",
        "skill3_lv": "8",
        "skill4_lv": "8",
        "mind_stats1": "치료효과 14%×",
        "mind_stats2": "공격력 20%×",
        "mind_skill1": "-",
        "mind_skill2": "-"
    },
    "모토하": {
        "role": "전격 서브 딜러",
        "tag":"전격 대미지보너스, 감전, TECHNICAL",
        "color": "#1e90ff",
        "main_revelation": ["신뢰"],
        "sub_revelation": ["변환"],
        "element": "전격",
        "element_weakness": "빙결",
        "element_resistance": "전격",
        "rarity": 4,
        "position": "지배",
        "release_order": 0,
        "persona": "아르빌다",
        "name": "아라이 모토하",
        "codename": "CLOSER",
        "sub_revel2": ["공격력", "대미지보너스"],
        "sub_revel3": ["크리티컬효과", "크리티컬확률"],
        "sub_revel4": ["공격력"],
        "sub_option1": ["크리티컬확률", "크리티컬효과"],
        "sub_option2": ["공격력", "대미지보너스","관통"],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": false,
            "LV12+5": false,
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "nan",
        "skill1_lv": "MAX",
        "skill2_lv": "8",
        "skill3_lv": "MAX",
        "skill4_lv": "MAX",
        "mind_stats1": "-",
        "mind_stats2": "-",
        "mind_skill1": "-",
        "mind_skill2": "-"
    },
    "모토하·여름": {
        "role": "축복 광역 딜러",
        "tag":"최대 생명",
        "color": "#1498fd",
        "main_revelation": ["결심"],
        "sub_revelation": ["미덕"],
        "element": "축복",
        "element_weakness": "핵열",
        "element_resistance": "축복",
        "rarity": 5,
        "position": "지배",
        "release_order": 11,
        "persona": "아르빌다",
        "name": "아라이 모토하·여름",
        "codename": "CLOSER",
        "sub_revel2": ["공격력", "대미지보너스"],
        "sub_revel3": ["크리티컬효과", "크리티컬확률"],
        "sub_revel4": ["공격력"],
        "sub_option1": ["생명", "크리티컬확률", "크리티컬효과"],
        "sub_option2": ["공격력", "대미지보너스"],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": "생명 16000",
            "LV10+5": "생명 16000",
            "LV12": false,
            "LV12+5": false,
            "LV13": "생명 16000",
            "LV13+5": "생명 16000"
        },
        "battle_plus_stats": "· 해명의힘 / 직책 8% / 마이팰리스 220 / 미나미 패시브 1500 / 미나미 스킬3 / 토모코·여름 찬란 1800",
        "skill1_lv": "MAX!",
        "skill2_lv": "8",
        "skill3_lv": "MAX!",
        "skill4_lv": "MAX!",
        "mind_stats1": "공격력 20%",
        "mind_stats2": "크리티컬효과 24%!",
        "mind_skill1": "1/HL!",
        "mind_skill2": "2/3!"
    },
    "몽타뉴": {
        "role": "빙결 단일 딜러",
        "tag":"동결, 추가 효과",
        "color": "#483d8b",
        "main_revelation": ["전념", "성장"],
        "sub_revelation": ["화려", "화려"],
        "element": "빙결",
        "element_weakness": "염동",
        "element_resistance": "빙결",
        "rarity": 4,
        "position": "반항",
        "release_order": 0,
        "persona": "테르프시코레",
        "name": "코토네 몽타뉴",
        "codename": "MONT",
        "sub_revel2": ["공격력", "대미지보너스"],
        "sub_revel3": ["크리티컬효과", "크리티컬확률"],
        "sub_revel4": ["공격력"],
        "sub_option1": ["크리티컬확률", "크리티컬효과"],
        "sub_option2": ["공격력", "대미지보너스"],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": false,
            "LV12+5": false,
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "nan",
        "skill1_lv": "8",
        "skill2_lv": "8",
        "skill3_lv": "8",
        "skill4_lv": "8",
        "mind_stats1": "-",
        "mind_stats2": "-",
        "mind_skill1": "-",
        "mind_skill2": "-"
    },
    "몽타뉴·백조": {
        "role": "질풍/빙결 단일 딜러",
        "tag":"폼 체인지, 추가 효과, 풍습, 동결",
        "color": "#483d8b",
        "main_revelation": ["전념", "자유"],
        "sub_revelation": ["화려", "개선"],
        "element": "질풍빙결",
        "element_weakness": "염동",
        "element_resistance": "질풍",
        "rarity": 5,
        "position": "반항",
        "release_order": 14,
        "persona": "테르프시코레",
        "name": "코토네 몽타뉴·백조",
        "codename": "MONT",
        "sub_revel2": ["공격력", "대미지보너스"],
        "sub_revel3": ["크리티컬효과", "크리티컬확률"],
        "sub_revel4": ["공격력"],
        "sub_option1": ["크리티컬확률", "크리티컬효과"],
        "sub_option2": ["공격력", "대미지보너스"],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": false,
            "LV12+5": false,
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "nan",
        "skill1_lv": "MAX!",
        "skill2_lv": "8",
        "skill3_lv": "MAX!",
        "skill4_lv": "MAX!",
        "mind_stats1": "공격력 20%",
        "mind_stats2": "크리티컬확률 12%!",
        "mind_skill1": "1/HL!",
        "mind_skill2": "2/3!"
    },
    "세이지": {
        "role": "질풍 단일 딜러",
        "tag":"풍습",
        "color": "#008080",
        "main_revelation": ["조화"],
        "sub_revelation": ["승리"],
        "element": "질풍",
        "element_weakness": "염동",
        "element_resistance": "질풍",
        "rarity": 4,
        "position": "반항",
        "release_order": 0,
        "persona": "레우코테아",
        "name": "시라토리 세이지",
        "codename": "FLEURET",
        "sub_revel2": ["공격력", "대미지보너스"],
        "sub_revel3": ["크리티컬효과", "크리티컬확률"],
        "sub_revel4": ["공격력"],
        "sub_option1": ["크리티컬확률", "크리티컬효과"],
        "sub_option2": ["공격력", "대미지보너스"],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": false,
            "LV12+5": false,
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "nan",
        "skill1_lv": "MAX",
        "skill2_lv": "8",
        "skill3_lv": "MAX!",
        "skill4_lv": "MAX",
        "mind_stats1": "공격력 20%",
        "mind_stats2": "크리티컬확률 12%!",
        "mind_skill1": "1/2",
        "mind_skill2": "3/HL!"
    },
    "슌": {
        "role": "빙결 탱커",
        "tag":"방어력 감소, 최대 생명 증가, 생명 회복, 공격받을 확률 증가, 총기, 동결",
        "color": "#4169e1",
        "main_revelation": ["여정"],
        "sub_revelation": ["주권"],
        "element": "빙결",
        "element_weakness": "축복",
        "element_resistance": "빙결",
        "rarity": 4,
        "position": "방위",
        "release_order": 0,
        "persona": "만돌란",
        "name": "카노 슌",
        "codename": "SOY",
        "sub_revel2": ["생명"],
        "sub_revel3": ["생명"],
        "sub_revel4": ["생명"],
        "sub_option1": ["생명"],
        "sub_option2": [],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": "생명 15600",
            "LV12+5": "생명 15600",
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "· 해명의힘 / 직책 8% / 마이팰리스 220 / 미나미 패시브 1500 / 미나미 스킬3 / 토모코·여름 찬란 1800",
        "skill1_lv": "8",
        "skill2_lv": "MAX!",
        "skill3_lv": "8",
        "skill4_lv": "8",
        "mind_stats1": "대미지감면 16%×",
        "mind_stats2": "생명 20%",
        "mind_skill1": "-",
        "mind_skill2": "-"
    },
    "아야카": {
        "role": "HIGHLIGHT 버퍼",
        "tag":"HIGHLIGHT, 공격력, 대미지보너스, 크리티컬확률, 생명 회복",
        "color": "#f3e966",
        "main_revelation": ["창조", "여정", "신뢰"],
        "sub_revelation": ["화해", "주권", "풍요"],
        "element": "전격",
        "element_weakness": "빙결",
        "element_resistance": "전격",
        "rarity": 5,
        "position": "우월",
        "release_order": 9,
        "persona": "칼리오페",
        "name": "사카이 아야카",
        "codename": "CHORD",
        "sub_revel2": ["공격력","생명","치료효과"],
        "sub_revel3": ["크리티컬확률", "공격력"],
        "sub_revel4": ["공격력","생명"],
        "sub_option1": ["공격력", "생명"],
        "sub_option2": ["크리티컬확률", "방어력"],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": "공격력 4067",
            "LV10+5": "공격력 4484",
            "LV12": false,
            "LV12+5": false,
            "LV13": "공격력 4317",
            "LV13+5": "공격력 4733"
        },
        "battle_plus_stats": "· 해명의힘 / 직책 8% / 마이팰리스 50",
        "skill1_lv": "MAX",
        "skill2_lv": "MAX!",
        "skill3_lv": "MAX!",
        "skill4_lv": "MAX!",
        "mind_stats1": "대미지감면 16%",
        "mind_stats2": "공격력 20%",
        "mind_skill1": "1/HL!",
        "mind_skill2": "2/3!"
    },
    "안": {
        "role": "화염 광역 딜러",
        "tag":"공격력 감소, 화상, TECHNICAL",
        "color": "#ff0000",
        "main_revelation": ["수락"],
        "sub_revelation": ["분쟁"],
        "element": "화염",
        "element_weakness": "빙결",
        "element_resistance": "화염",
        "rarity": 5,
        "position": "지배",
        "release_order": 1,
        "persona": "카르멘",
        "name": "타카마키 안",
        "codename": "PANTHER",
        "sub_revel2": ["공격력", "대미지보너스"],
        "sub_revel3": ["크리티컬효과", "크리티컬확률"],
        "sub_revel4": ["공격력"],
        "sub_option1": ["크리티컬확률", "크리티컬효과"],
        "sub_option2": ["공격력", "대미지보너스"],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": false,
            "LV12+5": false,
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "nan",
        "skill1_lv": "MAX",
        "skill2_lv": "8",
        "skill3_lv": "MAX!",
        "skill4_lv": "MAX!",
        "mind_stats1": "대미지보너스 16%",
        "mind_stats2": "크리티컬확률 12%!",
        "mind_skill1": "1/HL!",
        "mind_skill2": "2/3!"
    },
    "야오링": {
        "role": "광역 디버퍼",
        "tag":"방어력 감소, 받는 대미지 증가, 망각",
        "color": "#ff4500",
        "main_revelation": ["여정", "여정"],
        "sub_revelation": ["주권", "풍요"],
        "element": "주원",
        "element_weakness": "전격",
        "element_resistance": "주원",
        "rarity": 5,
        "position": "굴복",
        "release_order": 1,
        "persona": "맹파",
        "name": "리 야오링",
        "codename": "RIN",
        "sub_revel2": ["-"],
        "sub_revel3": ["-"],
        "sub_revel4": ["속도"],
        "sub_option1": ["속도"],
        "sub_option2": [],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": "속도 180",
            "LV10+5": "속도 183",
            "LV12": false,
            "LV12+5": false,
            "LV13": "속도 180",
            "LV13+5": "속도 200"
        },
        "battle_plus_stats": "· 마이팰리스 1 / 전용무기 : 스킬을 사용해 적 공격시 2턴 속도 15 증가",
        "skill1_lv": "MAX!",
        "skill2_lv": "8",
        "skill3_lv": "MAX!",
        "skill4_lv": "MAX",
        "mind_stats1": "효과명중 24%×",
        "mind_stats2": "공격력 20%",
        "mind_skill1": "1/HL",
        "mind_skill2": "2/3"
    },
    "야오링·사자무": {
        "role": "화염 광역 딜러",
        "tag":"변신, 화염 속성 TECHNICAL, 화상, 지속 대미지",
        "color": "#ff4500",
        "main_revelation": ["수락"],
        "sub_revelation": ["분쟁"],
        "element": "화염",
        "element_weakness": "질풍",
        "element_resistance": "화염",
        "rarity": 5,
        "position": "지배",
        "release_order": 19,
        "persona": "맹파·영춘",
        "name": "리 야오링·사자무",
        "codename": "RIN",
        "sub_revel2": ["공격력", "대미지보너스"],
        "sub_revel3": ["크리티컬효과", "크리티컬확률"],
        "sub_revel4": ["공격력"],
        "sub_option1": ["크리티컬효과"],
        "sub_option2": ["크리티컬확률"],
        "sub_option3": ["공격력", "대미지보너스"],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": false,
            "LV12+5": false,
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "nan",
        "skill1_lv": "MAX!",
        "skill2_lv": "8",
        "skill3_lv": "MAX!",
        "skill4_lv": "MAX!",
        "mind_stats1": "공격력 20%",
        "mind_stats2": "크리티컬효과 24%!",
        "mind_skill1": "1/HL!",
        "mind_skill2": "2/3!"
    },
    "유스케": {
        "role": "빙결 광역 딜러",
        "tag":"반격, 추가 효과, 방어력, 관통",
        "color": "#00bfff",
        "main_revelation": ["전념"],
        "sub_revelation": ["화려"],
        "element": "빙결",
        "element_weakness": "화염",
        "element_resistance": "빙결",
        "rarity": 5,
        "position": "지배",
        "release_order": 5,
        "persona": "고에몬",
        "name": "키타가와 유스케",
        "codename": "FOX",
        "sub_revel2": ["방어력", "대미지보너스"],
        "sub_revel3": ["크리티컬효과", "크리티컬확률"],
        "sub_revel4": ["방어력"],
        "sub_option1": ["크리티컬확률", "크리티컬효과"],
        "sub_option2": ["방어력", "대미지보너스"],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": false,
            "LV12+5": false,
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "nan",
        "skill1_lv": "8",
        "skill2_lv": "MAX",
        "skill3_lv": "MAX!",
        "skill4_lv": "MAX",
        "mind_stats1": "대미지보너스 16%",
        "mind_stats2": "크리티컬확률 12%!",
        "mind_skill1": "1/HL",
        "mind_skill2": "2/3!"
    },
    "유우미": {
        "role": "속성 대미지보너스 서포터",
        "tag":"공격력, 대미지보너스, 크리티컬확률",
        "color": "#9370db",
        "main_revelation": ["진정성", "신뢰"],
        "sub_revelation": ["직책", "힘"],
        "element": "보조",
        "element_weakness": "",
        "element_resistance": "",
        "rarity": 5,
        "position": "해명",
        "release_order": 6,
        "persona": "우라니아",
        "name": "시이나 유우미",
        "codename": "PHOEBE",
        "sub_revel2": ["공격력"],
        "sub_revel3": ["공격력"],
        "sub_revel4": ["공격력"],
        "sub_option1": ["공격력"],
        "sub_option2": ["크리티컬확률", "크리티컬효과"],
        "sub_option3": ["대미지보너스"],
        "minimum_stats": {
            "LV10": "공격력 5000",
            "LV10+5": "공격력 5500",
            "LV12": false,
            "LV12+5": false,
            "LV13": "공격력 6500",
            "LV13+5": "공격력 7000"
        },
        "battle_plus_stats": "· 마이팰리스 50",
        "skill1_lv": "MAX!",
        "skill2_lv": "MAX",
        "skill3_lv": "MAX!",
        "skill4_lv": "-",
        "mind_stats1": "생명 20%",
        "mind_stats2": "공격력 20%!",
        "mind_skill1": "1/2!",
        "mind_skill2": "3!"
    },
    "유이 YUI": {
        "role": "전격 단일 딜러",
        "tag":"추가 효과, 감전, 크리티컬확률",
        "color": "#8cd1cb",
        "main_revelation": ["성장"],
        "sub_revelation": ["변환"],
        "element": "전격",
        "element_weakness": "빙결",
        "element_resistance": "전격",
        "rarity": 5,
        "position": "반항",
        "release_order": 4,
        "persona": "압세데우스",
        "name": "YUI",
        "codename": "BUI",
        "sub_revel2": ["공격력", "대미지보너스"],
        "sub_revel3": ["크리티컬효과"],
        "sub_revel4": ["공격력"],
        "sub_option1": ["크리티컬확률", "크리티컬효과"],
        "sub_option2": ["공격력", "대미지보너스"],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": false,
            "LV12+5": false,
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "nan",
        "skill1_lv": "MAX!",
        "skill2_lv": "8",
        "skill3_lv": "MAX!",
        "skill4_lv": "MAX!",
        "mind_stats1": "공격력 20%",
        "mind_stats2": "크리티컬효과 24%!",
        "mind_skill1": "1/HL!",
        "mind_skill2": "2/3/추격!"
    },
    "유키미": {
        "role": "단일 실드",
        "tag":"축복, 실드, 대미지보너스, 방어력, 방어력 탈취",
        "color": "#642efe",
        "main_revelation": ["신뢰", "창조"],
        "sub_revelation": ["풍요", "화해"],
        "element": "축복",
        "element_weakness": "핵열",
        "element_resistance": "축복",
        "rarity": 4,
        "position": "방위",
        "release_order": 0,
        "persona": "스틱스",
        "name": "후지카와 유키미",
        "codename": "YUKI",
        "sub_revel2": ["방어력"],
        "sub_revel3": ["방어력"],
        "sub_revel4": ["방어력"],
        "sub_option1": ["-"],
        "sub_option2": [],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": "방어력 4500",
            "LV12+5": "방어력 4500",
            "LV13": false,
            "LV13+5": false,
        },
        "battle_plus_stats": "· 해명의힘 / 직책 8% / 마이팰리스 40 / 전용무기 : 3스 사용 시 적 방어력 탈취",
        "skill1_lv": "8",
        "skill2_lv": "MAX!",
        "skill3_lv": "MAX!",
        "skill4_lv": "MAX",
        "mind_stats1": "실드효과 14%×",
        "mind_stats2": "방어력 30%",
        "mind_skill1": "1/3",
        "mind_skill2": "2/HL"
    },
    "치즈코": {
        "role": "단일 디버퍼",
        "tag":"원소 이상, 방어력 감소, 받는 대미지 증가",
        "color": "#2680a7",
        "main_revelation": ["여정", "여정", "조화"],
        "sub_revelation": ["주권", "풍요", "진리"],
        "element": "핵열",
        "element_weakness": "화염",
        "element_resistance": "핵열",
        "rarity": 4,
        "position": "굴복",
        "release_order": 7,
        "persona": "암펠로스",
        "name": "나가오 치즈코",
        "codename": "VINO",
        "sub_revel2": ["-"],
        "sub_revel3": ["효과명중"],
        "sub_revel4": ["-"],
        "sub_option1": ["효과명중"],
        "sub_option2": ["생명", "방어력"],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": "효과명중 173%",
            "LV12+5": "효과명중 191%",
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "· 마이팰리스 2.3% / 의식3 : 동료가 원소이상 부여 시 2턴 10% 증가 (3중첩)",
        "skill1_lv": "MAX!",
        "skill2_lv": "8",
        "skill3_lv": "MAX!",
        "skill4_lv": "8",
        "mind_stats1": "효과명중 24%!",
        "mind_stats2": "공격력 20%",
        "mind_skill1": "1/HL",
        "mind_skill2": "2/3!"
    },
    "카스미": {
        "role": "축복 단일 딜러",
        "tag":"HIGHLIGHT, 변신, 예열",
        "color": "#9932cc",
        "main_revelation": ["지혜", "여정"],
        "sub_revelation": ["미덕", "주권"],
        "element": "축복",
        "element_weakness": "주원",
        "element_resistance": "축복",
        "rarity": 5,
        "position": "반항",
        "release_order": 17,
        "persona": "상드리옹",
        "name": "요시자와 카스미",
        "codename": "VIOLET",
        "sub_revel2": ["공격력", "대미지보너스"],
        "sub_revel3": ["크리티컬효과"],
        "sub_revel4": ["공격력"],
        "sub_option1": ["크리티컬효과"],
        "sub_option2": ["공격력", "대미지보너스"],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": false,
            "LV12+5": false,
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "nan",
        "skill1_lv": "MAX!",
        "skill2_lv": "MAX!",
        "skill3_lv": "MAX!",
        "skill4_lv": "MAX!",
        "mind_stats1": "대미지보너스 16%",
        "mind_stats2": "크리티컬확률 12%!",
        "mind_skill1": "1/2!",
        "mind_skill2": "3/HL!"
    },
    "카요": {
        "role": "공격력/효과 명중 서포터",
        "tag":"공격력, 효과 명중, 대미지보너스, 원소 이상, SP 회복",
        "color": "#8b008b",
        "main_revelation": ["진정성"],
        "sub_revelation": ["직책"],
        "element": "보조",
        "element_weakness": "",
        "element_resistance": "",
        "rarity": 4,
        "position": "해명",
        "release_order": 0,
        "persona": "클레오도라",
        "name": "토미야마 카요",
        "codename": "OKYANN",
        "sub_revel2": ["공격력"],
        "sub_revel3": ["공격력", "크리티컬확률"],
        "sub_revel4": ["공격력"],
        "sub_option1": ["공격력"],
        "sub_option2": ["크리티컬확률", "크리티컬효과"],
        "sub_option3": ["대미지보너스",
            "관통"],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": "공격력 5400",
            "LV12+5": "공격력 5850",
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "· 마이팰리스 50",
        "skill1_lv": "MAX!",
        "skill2_lv": "8",
        "skill3_lv": "MAX!",
        "skill4_lv": "-",
        "mind_stats1": "효과명중 28%",
        "mind_stats2": "공격력 20%",
        "mind_skill1": "2/3",
        "mind_skill2": "1"
    },
    "키라": {
        "role": "물리 단일 딜러",
        "tag":"변신, 지속 대미지, 관통",
        "color": "#ff69b4",
        "main_revelation": ["지혜"],
        "sub_revelation": ["억압"],
        "element": "물리",
        "element_weakness": "총격",
        "element_resistance": "물리",
        "rarity": 5,
        "position": "반항",
        "release_order": 10,
        "persona": "하르피이아",
        "name": "키타자토 키라",
        "codename": "MESSA",
        "sub_revel2": ["공격력"],
        "sub_revel3": ["공격력"],
        "sub_revel4": ["공격력"],
        "sub_option1": ["관통"],
        "sub_option2": ["공격력", "대미지보너스"],
        "sub_option3": ["효과명중"],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": false,
            "LV12+5": false,
            "LV13": "관통 31.5% (개조6)",
            "LV13+5": "관통 31.5% (개조6)"
        },
        "battle_plus_stats": "nan",
        "skill1_lv": "MAX!",
        "skill2_lv": "MAX",
        "skill3_lv": "MAX!",
        "skill4_lv": "MAX!",
        "mind_stats1": "공격력 20%",
        "mind_stats2": "관통 7.5%!",
        "mind_skill1": "1/HL!",
        "mind_skill2": "2/3!"
    },
    "키요시": {
        "role": "화염 서브딜러",
        "tag":"화상, 받는 대미지 증가, 화염 속성 TECHNICAL, TECHNICAL, 지속 대미지",
        "color": "#2e8b57",
        "main_revelation": ["여정", "여정"],
        "sub_revelation": ["주권", "풍요"],
        "element": "화염",
        "element_weakness": "질풍",
        "element_resistance": "화염",
        "rarity": 4,
        "position": "굴복",
        "release_order": 0,
        "persona": "슈케",
        "name": "쿠로타니 키요시",
        "codename": "KEY",
        "sub_revel2": ["생명"],
        "sub_revel3": ["생명"],
        "sub_revel4": ["생명"],
        "sub_option1": ["생명"],
        "sub_option2": [],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": false,
            "LV12+5": false,
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "nan",
        "skill1_lv": "8",
        "skill2_lv": "8",
        "skill3_lv": "8",
        "skill4_lv": "8",
        "mind_stats1": "-",
        "mind_stats2": "-",
        "mind_skill1": "-",
        "mind_skill2": "-"
    },
    "토모코": {
        "role": "단일 버퍼",
        "tag":"공격력, 효과 명중, 수면, 받는 대미지 증가",
        "color": "#cc5995",
        "main_revelation": ["신뢰", "신뢰", "창조"],
        "sub_revelation": ["풍요", "힘", "화해"],
        "element": "염동",
        "element_weakness": "주원",
        "element_resistance": "염동",
        "rarity": 4,
        "position": "우월",
        "release_order": 0,
        "persona": "프로심나",
        "name": "노게 토모코",
        "codename": "MOKO",
        "sub_revel2": ["공격력","생명"],
        "sub_revel3": ["공격력","생명"],
        "sub_revel4": ["공격력","생명"],
        "sub_option1": ["생명", "방어"],
        "sub_option2": ["공격력"],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": "공격력 4064",
            "LV12+5": "공격력 4464",
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "· 해명의힘 / 직책 8% / 마이팰리스 50",
        "skill1_lv": "8",
        "skill2_lv": "MAX",
        "skill3_lv": "MAX",
        "skill4_lv": "8",
        "mind_stats1": "효과명중 28%×",
        "mind_stats2": "공격력 20%",
        "mind_skill1": "-",
        "mind_skill2": "-"
    },
    "토모코·여름": {
        "role": "광역 힐러/디버퍼",
        "tag":"공격력, 효과 명중, 받는 대미지 증가, 최대 생명 증가, 방어력 감소, 추가 효과",
        "color": "#cc5995",
        "main_revelation": ["조화", "신뢰", "진정성"],
        "sub_revelation": ["승리", "풍요", "환희"],
        "element": "염동",
        "element_weakness": "주원",
        "element_resistance": "염동",
        "rarity": 5,
        "position": "구원",
        "release_order": 11,
        "persona": "프로심나",
        "name": "노게 토모코·여름",
        "codename": "MOKO",
        "sub_revel2": ["치료효과"],
        "sub_revel3": ["공격력", "크리티컬확률"],
        "sub_revel4": ["공격력"],
        "sub_option1": ["공격력", "대미지보너스"],
        "sub_option2": ["크리티컬확률", "크리티컬효과"],
        "sub_option3": ["SP회복"],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": false,
            "LV12+5": false,
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "nan",
        "skill1_lv": "8",
        "skill2_lv": "MAX!",
        "skill3_lv": "MAX!",
        "skill4_lv": "8",
        "mind_stats1": "SP회복 125%",
        "mind_stats2": "공격력 20%",
        "mind_skill1": "1/2",
        "mind_skill2": "3/HL"
    },
    "토시야": {
        "role": "주원 단일 딜러",
        "tag":"주원, 지속 대미지",
        "color": "#4682b4",
        "main_revelation": ["지혜"],
        "sub_revelation": ["억압"],
        "element": "주원",
        "element_weakness": "전격",
        "element_resistance": "주원",
        "rarity": 4,
        "position": "반항",
        "release_order": 0,
        "persona": "고르귀라",
        "name": "스미 토시야",
        "codename": "SEPIA",
        "sub_revel2": ["-"],
        "sub_revel3": ["효과명중"],
        "sub_revel4": ["-"],
        "sub_option1": ["효과명중"],
        "sub_option2": ["대미지보너스"],
        "sub_option3": ["생명", "방어"],
        "minimum_stats": {
            "LV10": false,
            "LV10+5": false,
            "LV12": false,
            "LV12+5": false,
            "LV13": false,
            "LV13+5": false
        },
        "battle_plus_stats": "nan",
        "skill1_lv": "MAX",
        "skill2_lv": "8",
        "skill3_lv": "8",
        "skill4_lv": "8",
        "mind_stats1": "-",
        "mind_stats2": "-",
        "mind_skill1": "-",
        "mind_skill2": "-"
    },
    "하루": {
        "role": "염동 광역 딜러",
        "tag":"총격, 예열",
        "color": "#800080",
        "main_revelation": ["지혜"],
        "sub_revelation": ["환희"],
        "element": "염동",
        "element_weakness": "핵열",
        "element_resistance": "염동",
        "rarity": 5,
        "position": "지배",
        "release_order": 15,
        "persona": "밀라디",
        "name": "오쿠무라 하루",
        "codename": "NOIR",
        "sub_revel2": ["대미지보너스"],
        "sub_revel3": ["크리티컬확률", "효과명중"],
        "sub_revel4": ["공격력"],
        "sub_option1": ["효과명중"],
        "sub_option2": ["크리티컬확률", "크리티컬효과"],
        "sub_option3": ["공격력", "대미지보너스"],
        "minimum_stats": {
            "LV10": "효과명중 140% / 210%",
            "LV10+5": "효과명중 140% / 210%",
            "LV12": false,
            "LV12+5": false,
            "LV13": "효과명중 140% / 210%",
            "LV13+5": "효과명중 140% / 210%"
        },
        "battle_plus_stats": "· 마이팰리스 2.3%",
        "skill1_lv": "MAX!",
        "skill2_lv": "8",
        "skill3_lv": "MAX!",
        "skill4_lv": "MAX!",
        "mind_stats1": "공격력 20%",
        "mind_stats2": "크리티컬효과 24%!",
        "mind_skill1": "3/HL!",
        "mind_skill2": "1/2"
    },
    "하루나": {
        "role": "광역 버퍼",
        "tag":"공격력, 대미지보너스",
        "color": "#ff1493",
        "main_revelation": ["창조", "신뢰"],
        "sub_revelation": ["화해", "풍요"],
        "element": "염동",
        "element_weakness": "주원",
        "element_resistance": "염동",
        "rarity": 5,
        "position": "우월",
        "release_order": 0,
        "persona": "다에이라",
        "name": "니시모리 하루나",
        "codename": "RIDDLE",
        "sub_revel2": ["공격력","생명"],
        "sub_revel3": ["공격력","생명"],
        "sub_revel4": ["공격력","생명"],
        "sub_option1": ["공격력"],
        "sub_option2": [],
        "sub_option3": [],
        "minimum_stats": {
            "LV10": "공격력 4880",
            "LV10+5": "공격력 5380",
            "LV12": false,
            "LV12+5": false,
            "LV13": "공격력 5180",
            "LV13+5": "공격력 5680"
        },
        "battle_plus_stats": "· 해명의힘 / 직책 8% / 마이팰리스 50 / 의식1 : 동료가 대미지 부여시 20% 증가 (3중첩) / 전용무기 : 동심 소모 시 공격력 증가",
        "skill1_lv": "8",
        "skill2_lv": "8",
        "skill3_lv": "MAX!",
        "skill4_lv": "MAX",
        "mind_stats1": "대미지보너스 16%",
        "mind_stats2": "공격력 20%",
        "mind_skill1": "1/HL",
        "mind_skill2": "2/3"
    },
    "후타바": {
        "role": "약점 강화 서포터",
        "tag":"약점 변경, 방어력 감소, 공격력, 받는 대미지 증가, 크리티컬확률, 실드",
        "color": "#32cd32",
        "main_revelation": ["결심", "진정성"],
        "sub_revelation": ["직책", "직책"],
        "element": "보조",
        "element_weakness": "",
        "element_resistance": "",
        "rarity": 5,
        "position": "해명",
        "release_order": 12,
        "persona": "네크로노미콘",
        "name": "사쿠라 후타바",
        "codename": "NAVI",
        "sub_revel2": ["공격력"],
        "sub_revel3": ["크리티컬효과", "크리티컬확률"],
        "sub_revel4": ["공격력"],
        "sub_option1": ["공격력"],
        "sub_option2": ["크리티컬확률", "크리티컬효과"],
        "sub_option3": ["대미지보너스"],
        "minimum_stats": {
            "LV10": "공격력 4600",
            "LV10+5": "공격력 5060",
            "LV12": false,
            "LV12+5": false,
            "LV13": "공격력 5980",
            "LV13+5": "공격력 6440"
        },
        "battle_plus_stats": "· 마이팰리스 50",
        "skill1_lv": "MAX!",
        "skill2_lv": "MAX",
        "skill3_lv": "MAX!",
        "skill4_lv": "-",
        "mind_stats1": "생명 20%!",
        "mind_stats2": "공격력 20%!",
        "mind_skill1": "1/2!",
        "mind_skill2": "3!"
    }
};