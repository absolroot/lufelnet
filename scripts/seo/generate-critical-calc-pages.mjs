...
/**
 * generate-critical-calc-pages.mjs
 *
 * Generates static SEO pages for the critical-calc app in each supported language.
 *
 * Usage:
 *   node scripts/seo/generate-critical-calc-pages.mjs
 *   node scripts/seo/generate-critical-calc-pages.mjs --check
 */
...
const OUTPUT_DIR = path.join(ROOT, 'pages', 'critical-calc');
const LANGS = ['kr', 'en', 'jp'];
const IMAGE_PATH = '/assets/img/home/SEO.png';

const seoMetaPath = path.join(ROOT, 'i18n', 'pages', 'critical-calc', 'seo-meta.json');
...
function ensureSeoMetaShape(meta) {
  for (const lang of LANGS) {
    ...
    if (typeof langMeta.title !== 'string' || !langMeta.title) {
      throw new Error(`i18n/pages/critical-calc/seo-meta.json missing valid title for ${lang}.`);
    }
    if (typeof langMeta.description !== 'string' || !langMeta.description) {
      throw new Error(`i18n/pages/critical-calc/seo-meta.json missing valid description for ${lang}.`);
    }
  }
}

function renderCriticalCalcPage({ lang, title, description }) {
  const permalink = `/${lang}/critical-calc/`;
  const altKo = '/kr/critical-calc/';
  const altEn = '/en/critical-calc/';
  const altJp = '/jp/critical-calc/';
  ...
  return [
    ...
    'custom_data: [tooltip.js, calc/critical-data.js, /data/character_info.js]',
    ...
    `  jp: ${altJp}`,
    '---',
    '{% include critical-calc-body.html %}',
    ''
  ].join('\n');
}
...
function buildExpectedFiles(seoMeta) {
  ...
  for (const lang of LANGS) {
    ...
    const content = normalizeNewline(
      renderCriticalCalcPage({ lang, title, description })
    );
...
function runCheck(expectedFiles) {
  ...
  if (missing.length || changed.length || extra.length) {
    console.error('Critical-calc SEO pages are out of date.');
    if (missing.length) {
  ...
  console.log(`Critical-calc SEO pages are up to date (${expectedFiles.size} files).`);
}
...
function runGenerate(expectedFiles) {
  ...
  console.log(`Generated ${expectedFiles.size} critical-calc SEO files in ${toPosix(path.relative(ROOT, OUTPUT_DIR))}`);
}
