...
function getCarouselStudioEffectiveRule() {
  // An explicit 0 is a valid position/scale value, so merge only missing CSS
  // declarations instead of using truthiness (which would silently discard 0).
  const hasValue = (value) => value !== '' && value != null;
  return getCarouselStudioRules().reduce((effective, rule) => ({
    selector: rule.selector,
    top: hasValue(rule.top) ? rule.top : effective.top,
    right: hasValue(rule.right) ? rule.right : effective.right,
    scale: hasValue(rule.scale) ? rule.scale : effective.scale,
    transform: hasValue(rule.transform) ? rule.transform : effective.transform
  }), { selector: '', top: '', right: '', scale: '', transform: '' });
}
...
function bindEvents() {
  ...
  dom.btnCarouselStudioApply?.addEventListener('click', async () => {
    try {
      // Preserve the form's final-scale draft before sending its CSS-scale form.
      await saveCarouselStudioDraft();
      const result = await requestJson('/api/carousel-studio/apply', { method: 'POST', body: carouselStudioCssDraftFromForm() });
      state.carouselStudio.rules = result.rules || state.carouselStudio.rules;
      loadCarouselStudioRuleValues();
      if (dom.carouselStudioStatus) dom.carouselStudioStatus.textContent = `CSS 반영 완료: ${result.selector}`;
    } catch (error) { if (dom.carouselStudioStatus) dom.carouselStudioStatus.textContent = error.message; }
  });
