window.ritualData = window.ritualData || {};
window.enCharacterRitualData = window.enCharacterRitualData || {};
window.jpCharacterRitualData = window.jpCharacterRitualData || {};
window.cnCharacterRitualData = window.cnCharacterRitualData || {};

window.ritualData["코로마루"] = {
  "name": "코로마루",
  "r0": "도시의 인기 스타",
  "r0_detail": "전투 시 첫 캐릭터 행동 턴 시작 전, 코로마루가 수동으로 동료 1명을 『주인』으로 지정하고, 『특정 규칙』에 따라 『주인』과 『사냥개 협공』을 진행할 수 있다. 『주인』 선택 완료 후 코로마루는 자동으로 지원 스킬을 시전한다. 코로마루의 테우르기아는 목표에게 『영혼 추격』을 추가할 수 있으며, 자신의 다른 페르소나 스킬을 사용해 『영혼 추격』을 보유한 유닛 공격 시 목표를 화염/전격/빙결 속성 『적합성 하락』 상태에 빠뜨린다. 또한 아군 전체에 『충견의 포효』를 시전한다. 『충견의 포효』: 2턴 동안 공격력이 15% 증가하고, 대미지가 15% 증가하며, 테우르기아 대미지 증폭이 20% 증가한다.",
  "r1": "멍!",
  "r1_detail": "『주인』과 『사냥개 협공』을 완료하거나 자신이 페르소나 스킬을 시전하면, 아군 전체의 크리티컬 효과가 25% 영구 증가한다(2회 중첩 가능). 『주인』이 치명 대미지를 받을 때 주인의 HP가 1포인트 미만으로 떨어지지 않게 함과 동시에, 최대 생명의 50%를 회복한다. 전투당 1회로 제한된다.",
  "r2": "길고 긴 기다림",
  "r2_detail": "『적합성 저하』 상태 추가 시 목표를 추가로 화상, 감전, 동결, 주원 상태에 빠뜨릴 수 있다. 『충견의 포효』는 추가로 공격력을 60%, 방어력을 30% 증가시킨다.",
  "r3": "어둠 속 잠행",
  "r3_detail": "『칠흑의 지옥불』, 『은빛 달의 서리』의 스킬 레벨이 3레벨 증가하고, 최대 15레벨까지 증가한다.",
  "r4": "월영의 포효",
  "r4_detail": "테우르기아: 주는 주원 대미지가 내성을 무시하고, 추가로 목표의 받는 대미지가 20% 영구 증가한다.",
  "r5": "영혼 동조",
  "r5_detail": "『붉은 섬광』, 『전투 기술』의 스킬 레벨이 3레벨 증가하고, 최대 15레벨까지 증가한다.",
  "r6": "용감한 전사",
  "r6_detail": "코로마루의 턴 시작 시 『저승의 판결』 효과가 자동으로 발동해, 필드에 있는 적 전체를 동시에 화염, 전격, 빙결 속성의 『적합성 저하』 상태에 빠뜨린다. 추가로 목표가 받는 화염, 전격, 빙결 약점 대미지를 20% 증가시키고, 받는 화염, 전격, 빙결 대미지를 20% 증가시킨다. 『충견의 포효』 효과 중첩 상한이 2회까지 증가한다."
};

window.enCharacterRitualData["코로마루"] = {
  "name": "Koromaru",
  "r0": "City's Top Dog",
  "r0_detail": "At the start of battle, Koromaru can manually select 1 ally as the [Owner] and can use [Hound's Hunt] based on [Specific Conditions]. After selecting the [Owner], Koromaru will automatically use Assist Skill. \nKoromaru's Theurgy can inflict targets with [Soul Hunt]. Inflict targets with Fire/Elec/Ice [Attribute Downgrade] when using certain Persona skills to attack foes with [Soul Hunt], then grant all allies [Loyal Howl]: Increase Attack by 15%, damage by 15%, and Theurgy Damage Amp by 20%. Lasts for 2 turns.\n  [Specific Conditions]: When selecting a S.E.E.S. member as the [Owner], they can use the Assist Skill menu to command Koromaru to use Persona skills. They can use other Assist Skills as normal. When not selecting a S.E.E.S. member as the [Owner], at the start of their turn, Koromaru will automatically use Persona skills based on their attribute. After selecting the [Owner], Koromaru will no longer have his own action, but can still activate Koromaru's start/end of turn effects at the [Owner]'s start/end of turn.\n[Attribute Downgrade]: Null, Repel, and Drain become Resist, Resist becomes Normal, and Normal becomes Weak. Weak attributes take 25% more weakness damage.",
  "r1": "Woof!",
  "r1_detail": "After [Hound's Hunt] or after Koromaru manually uses a Persona skill, permanently increase all allies' critcal damage by 25%. Stacks up to 2 times.\nWhen the [Owner] takes damage that would KO them, survive the attack with 1 HP and restore 50% max HP to them. This effect activates once per battle.",
  "r2": "Lasting Wait",
  "r2_detail": "When inflicting [Attribute Downgrade], inflict targets with Burn, Shock, Freeze, and Curse. \n[Loyal Howl] increase Attack by 60% and Defense by 30% more.",
  "r3": "Night Stalker",
  "r3_detail": "Increase the skill levels of Black Inferno and Silver Frost by 3.",
  "r4": "Moonlight Howling",
  "r4_detail": "Theurgy: When dealing Curse damage, ignore foes' Curse resistance, permanently increase targets' damage taken by 20%.",
  "r5": "Soul Sync",
  "r5_detail": "Increase the skill levels of Scarlet Flash and Thief Tactics by 3.",
  "r6": "Brave Warrior",
  "r6_detail": "At the start of Koromaru's turn, automatically activate [Judgment of Hades]: Inflict all foes with Fire Electric Ice [Attribute Downgrade], additionally increase target's Fire Electric Ice weakness damage taken by 20% and Fire Electric Ice damage taken by 20%.\nIncrease the maximum stacks of [Loyal Howl] to 2."
};

window.jpCharacterRitualData["코로마루"] = {
  "name": "コロマル",
  "r0": "街の人気者",
  "r0_detail": "戦闘開始時、味方1体を『主人』に指定して『猟犬協撃』を行う。その後、サポートスキルを自動発動する。テウルギアで『魂追猟』を付与し、攻撃時に対象を火炎/電撃/氷結の『適応性降級』状態にする。味方全体に2ターンの間『忠犬の咆哮』を付与する。\n『忠犬の咆哮』：攻撃力が15%、ダメージが15%、テウルギアダメージが20%上昇する。\nコロマルは『主人』を選択した後、自身のターンを持たなくなるが、『主人』のターン開始/終了時に自身のターン開始/終了効果を発動する。\n『主人』がS.E.E.S.メンバーの場合：『主人』はサポートスキル欄の指令でコロマルにペルソナスキルを使用させることができ、その後も従来どおり他のサポートスキルを使用できる。\n『主人』が非S.E.E.S.メンバーの場合：『主人』のターン開始時、コロマルは『主人』の属性に基づいて自動でペルソナスキルを使用する。",
  "r1": "ワン！",
  "r1_detail": "『主人』と『猟犬協撃』を完了するか、自身が能動的にペルソナスキルを発動した後、味方全体は永続でCRT倍率+25%を獲得する（最大2重複）。『主人』が致命ダメージを受けた時、HPが1未満にならず、最大HPの50%を回復する（1戦闘につき1回まで）。",
  "r2": "長鳴きの守り",
  "r2_detail": "『適応性降級』付与時、追加で対象を燃焼・感電・凍結・呪い状態にする。『忠犬の咆哮』はさらに攻撃力を60%、防御力を30%上昇させる。",
  "r3": "暗夜潜行",
  "r3_detail": "『漆黒の地獄火』、『銀の月霜』のスキルレベルが3上昇し、最大15レベルまで上昇する。",
  "r4": "月影の咆哮",
  "r4_detail": "テウルギア：与える呪怨ダメージが適応性を無視し、さらに対象の被ダメージを永続的に20%上昇させる。",
  "r5": "魂の同調",
  "r5_detail": "『レッドフラッシュ』、『戦闘技巧』のスキルレベルが3上昇し、最大15レベルまで上昇する。",
  "r6": "勇敢なる戦士",
  "r6_detail": "コロマルのターン開始時、自動で『冥府の裁決』効果を発動する：戦場の敵全体を同時に火炎・電撃・氷結属性の『適応性降級』状態にし、さらに対象が受ける火炎・電撃・氷結の弱点ダメージを20%上昇させ、受ける火炎・電撃・氷結ダメージを20%上昇させる。『忠犬の咆哮』の重複上限が2層に増加する。"
};

window.cnCharacterRitualData["코로마루"] = {
  "name": "虎狼丸",
  "r0": "城市人气王",
  "r0_detail": "战斗首个角色行动回合开始前，虎狼丸手动指定一名同伴为 『饲主』，可按照『特定规则』与『饲主』进行『猎犬协击』。完成『饲主』选择后，虎狼丸会自动释放援助技能。虎狼丸的神通法可为目标添加『灵魂追猎』，通过自身不同人格面具技能攻击拥有『灵魂追猎』单位时可使目标陷入火焰/电击/冰冻属性『适应性降级』状态，并为所有同伴施加 『忠犬的咆哮』：攻击力提升15%，伤害提升15%，神通法伤害增幅提升20%，持续2回合。",
  "r1": "汪！",
  "r1_detail": "与『饲主』完成『猎犬协击』或在自身主动释放人格面具技能后，使所有同伴永久获得25%的暴击效果提升，效果可叠加2层。『饲主』受到致命伤害时，使其血量不低于1点并为其恢复50%的最大生命值，每场战斗限1次。",
  "r2": "长鸣的守候",
  "r2_detail": "添加『适应性降级』状态时可额外使目标陷入燃烧、触电、冻结、诅咒状态；『忠犬的咆哮』额外使攻击力提升60%，防御力提升30%。",
  "r3": "暗夜潜行",
  "r3_detail": "『漆黑地狱火』、『银之月霜』的技能等级提升3级，至多提升至15级。",
  "r4": "月影咆哮",
  "r4_detail": "神通法：造成的诅咒伤害无视耐性，额外使目标受到伤害永久提升20%。",
  "r5": "灵魂同调",
  "r5_detail": "『红闪击』、『战斗技巧』的技能等级提升3级，至多提升至15级。",
  "r6": "勇敢的战士",
  "r6_detail": "虎狼丸回合开始时，自动触发『冥府裁决』效果：使场上所有敌人同时陷入火焰、电击、冰冻属性的『适应性降级』状态，额外使目标受到的火焰、电击、冰冻弱点伤害提升20%，受到火焰、电击、冰冻伤害提升20%； 『忠犬的咆哮』效果叠加上限提升至2层。"
};

